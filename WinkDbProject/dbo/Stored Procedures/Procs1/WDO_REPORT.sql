...
BEGIN
	...
	IF(@CAMPAIGN_ID = 145)
		BEGIN
		SELECT * from(
		SELECT ROW_NUMBER() OVER (Order by wto.cus_date ASC)AS no, wto.order_number, wto.cus_id as customer_id, 
		wto.cus_date, wto.cus_ip, wto.cus_location, wto.mer_ip, wto.mer_location, wto.mer_date,
		wto.completion, wto.validity, wto.points, wto.exception, cus.first_name +' '+cus.last_name as customer_name,cus.email, cus.status,
