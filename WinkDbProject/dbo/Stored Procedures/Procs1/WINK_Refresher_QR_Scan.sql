CREATE PROC [dbo].[WINK_Refresher_QR_Scan]          
...
IF(@winktagId = 142 or @winktagId = 102 )
BEGIN
	set @default_image_id =304
END
ELSE IF(@winktagId = 103)
BEGIN
	set @default_image_id =305
END
...
--IF((SELECT special_campaign from asset_type_management where qr_code_value like @qrcode) like 'No'  AND @qrcode not like 'Train%' AND @qrcode not like '%VendingMachine%' AND @qrcode not like 'WINKSimplyGo_UOBMC_01_50733')
--BEGIN
...
--	AND qr_code not like 'WINKSimplyGo_UOBMC_01_50733'
--	AND qr_code in (
...
SELECT @SCAN_INTERVAL=SCAN_INTERVAL, @SCAN_VALUE = SCAN_VALUE,@Valid = 1 FROM 
	...
	BEGIN  -- 4.. Check customer is enable
		...
		Cast(@CURRENT_DATETIME as date) <= CAST(s.scan_end_date as date)
		 ...
		 BEGIN
			...
			IF(Cast(@CURRENT_DATETIME as datetime) between '2020-01-23 10:00:00.000' AND '2020-06-23 16:30:00.000')
			BEGIN
				...
				VALUES
				(@customer_id,@winktagId,@qrcode, @GPS_location,@ip_address,(SELECT TODAY FROM VW_CURRENT_SG_TIME), '0');
				...
				BEGIN
					IF(@winktagId = 142 or @winktagId = 102)
					BEGIN
						SET @RETURN_NO='000' -- quiz                           
						GOTO Err
					END
					ELSE IF(@winktagId = 103)
					BEGIN
						SET @RETURN_NO='010' -- viral leaders game                         
						GOTO Err
					END
				END
			END
	...
	ELSE IF @RETURN_NO='000' -- Refresher quiz
	...
	ELSE IF @RETURN_NO='010' -- Viral Leaders Game

	BEGIN 
	 SELECT '2' as response_code,'Success' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
		, 3 as timer_interval_second,@default_image_id as image_id
		RETURN 
	END
