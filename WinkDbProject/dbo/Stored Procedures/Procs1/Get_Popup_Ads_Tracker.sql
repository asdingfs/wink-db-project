CREATE PROCEDURE [dbo].[Get_Popup_Ads_Tracker]
(
  @url_name varchar(50),
...
from popup_ads_tracker as t
join popup_ads_app as p
on t.url_id = p.id
left join 
customer as c
...
from 
(select * from popup_ads_tracker where CAST(created_at as date) BETWEEN 
CAST(@from_date as date) and CAST(@to_date as date)
and (@ip_address is null or popup_ads_tracker.ip_address like '%'+@ip_address+'%')
