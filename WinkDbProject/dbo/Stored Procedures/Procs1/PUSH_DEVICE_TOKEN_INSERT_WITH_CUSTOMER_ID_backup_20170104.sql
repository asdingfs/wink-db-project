CREATE PROC [dbo].[PUSH_DEVICE_TOKEN_INSERT_WITH_CUSTOMER_ID_backup_20170104]
...
BEGIN
	...
	ELSE IF EXISTS (SELECT * FROM push_device_token WHERE customer_id = @customer_id and (WID is not null and WID !='') and (customer_id != 0 and customer_id is not null and customer_id != ''))
		SET @WID = (SELECT TOP 1 WID FROM push_device_token WHERE customer_id = @customer_id and (WID is not null and WID !='') and (customer_id != 0 and customer_id is not null and customer_id != '') ORDER BY updated_at DESC)
