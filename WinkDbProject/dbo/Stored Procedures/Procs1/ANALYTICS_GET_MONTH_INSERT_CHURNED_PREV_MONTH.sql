CREATE PROC [dbo].[ANALYTICS_GET_MONTH_INSERT_CHURNED_PREV_MONTH]
...
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM analytics_pre_month_churned WHERE period = @date)
	BEGIN

		insert into analytics_pre_month_churned (customer_id,period,create_at)

		select distinct active_user.customer_id,@date,(select * from VW_CURRENT_SG_TIME) from
		(
		...
		--#########################get current churned customers to exclude in trip###################################
		truncate table customer_churned_temp

		insert into customer_churned_temp (customer_id,period,create_at)
		(select customer_id,period,(select today from VW_CURRENT_SG_TIME) from analytics_pre_month_churned where period = @date)
