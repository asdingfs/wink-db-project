...
CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_Mediaparty2017]          
...
If CAST(@CURRENT_DATETIME as time) < '21:55:00' OR CAST(@CURRENT_DATETIME as time) > '23:59:00'
--If CAST(@CURRENT_DATETIME as time) < '16:00:00' and CAST(@CURRENT_DATETIME as time) > '03:00:00'  
BEGIN    
...
IF @qrcode = 'XCOTradeParty_LuckyDraw_01_49261'
BEGIN
	IF EXISTS (SELECT * FROM hof_luckydraw WHERE customer_id = (SELECT CUSTOMER_ID FROM customer WHERE auth_token = @customer_tokenid) AND event_name = 'XCO Trade Party 2017')
	BEGIN
		SELECT '0' as response_code, 'Invalid Scan' as response_message , 3 as timer_interval_second
		RETURN	
	END

	ELSE IF (SELECT COUNT(*) FROM customer_earned_points WHERE qr_code = 'XCOTradeParty_LuckyDraw_01_49261') = 1
	BEGIN
		SELECT '0' as response_code, 'Invalid Scan' as response_message , 3 as timer_interval_second
		RETURN
	END
...
-- End Safeguard----------------------------------------------------------------------------
	...
	BEGIN  
		SELECT '0' as response_code, 'One scan per user' as response_message , 3 as timer_interval_second
		RETURN 
	END
	...
	BEGIN  
		...
		--SET @WINNER_COUNT = (SELECT qty FROM wink_luckydraw WHERE qr_code = @qrcode)
		SET @PRIZE = (SELECT PRIZE FROM wink_luckydraw WHERE qr_code = @qrcode)

		IF EXISTS (SELECT * FROM wink_luckydraw WHERE qr_code = @qrcode)
		BEGIN
			IF NOT EXISTS (SELECT * FROM hof_luckydraw WHERE customer_id = @CUSTOMER_ID AND event_name = 'XCO Trade Party 2017')
			BEGIN
				IF ((SELECT COUNT(*) FROM hof_luckydraw WHERE qr_code = @qrcode) < (SELECT qty FROM wink_luckydraw WHERE qr_code = @qrcode))
				BEGIN
					SET @WINNER = 'Congrats! You have won '+ @prize
	    ...
	    If(@@ERROR=0)
	    Begin
	    SET @SMALL_IMAGE_URL = 'xcodefault.png'
		SET @SMALL_WEBSITE_URL = 'https://www.winkwink.sg'

		IF(@qrcode = 'XCOTradeParty_XCOtradeparty_01_49191')
			SET @SMALL_IMAGE_URL = 'xco500.png'

		--XCOTradeParty_XCOtradeparty_01_49191
