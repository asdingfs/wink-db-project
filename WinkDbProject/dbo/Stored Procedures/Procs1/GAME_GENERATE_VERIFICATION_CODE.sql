CREATE PROC GAME_GENERATE_VERIFICATION_CODE
(
	@merchant_id int,
	@event_id int
)
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						WHILE EXISTS(SELECT * FROM Game_eVoucher_verification WHERE verification_code = @verification_code)
						BEGIN
							SET @verification_code = CONVERT(numeric(9,0),rand() * 899999999) + 100000000
						END
					...
					SELECT '1' AS response_code,'Verification code successfully created' as response_message
					return;
				END
				ELSE
				BEGIN
					SELECT '0' AS response_code,'Event ID '+ convert(varchar(10),@event_id)+' already exists' as response_message
					return;
				END
			END
			ELSE
