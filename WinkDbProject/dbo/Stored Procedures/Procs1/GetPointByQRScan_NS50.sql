CREATE PROC [dbo].[GetPointByQRScan_NS50]          
...
--- Check 10,000 Scan
    ...
    IF((select count(*) from customer_earned_points where qr_code ='NS50_NS50_01_49312') >= 4)
	BEGIN
	 SELECT '0' as response_code, 'Fully redeemed' as response_message , 3 as timer_interval_second
		RETURN 
	END
...
-- 2. Check Account Locked
    IF (@CUSTOMER_ID = 0 or @CUSTOMER_ID ='' or @CUSTOMER_ID is null)
	 ...
	 SELECT '0' as response_code, 'Account locked.Please contact customer service.' as response_message , 3 as timer_interval_second
		RETURN 
	END-- END
...
select *
from (
select top 50  customer_id,GPS_location from customer_earned_points  
...
--select * from customer where customer.email like '%nnk%'
