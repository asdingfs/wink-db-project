...
CREATE PROCEDURE [dbo].[Create_New_MerchantPartners_With_Industry4]
...
BEGIN
    ...
    Insert into merchant_partners
        (name,created_at,updated_at,status,industry_id,merchant_logo_app,url,link_to_website_status,description)
    values
        (@merchant_name, @current_date, @current_date, '1', @industry_id, @merchant_logo, @url, @link_to_website_status, @description)
    ...
    IF @merchant_id >0 
        ...
        IF (@outlet_addresss IS NOT NULL AND @outlet_addresss !='')
            ...
            Insert into merchant_partners_address
                (merchant_id , outlet_address,postal_code,phone,created_at,updated_at,status)
            values
                (@merchant_id, @outlet_addresss, @postal_code, @phone, @current_date, @current_date, '1')
