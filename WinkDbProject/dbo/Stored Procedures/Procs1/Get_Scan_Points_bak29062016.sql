CREATE PROC [dbo].[Get_Scan_Points_bak29062016]          
...
BEGIN
IF EXISTS (Select * from customer where auth_token = @customer_tokenid and (phone_no!='' and phone_no is not null))
...
BEGIN
  ...
  BEGIN --- Begin check time between 00:30 and 05:30
	...
	BEGIN                                              
		SELECT '0' as response_code, 'Invalid Scan' as response_message , 2 as timer_interval_second
	...
	BEGIN  
		SELECT '0' as response_code, 'Invalid QR Code' as response_message , 2 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='004' 
	BEGIN  
		SELECT '0' as response_code, 'Insert Fail' as response_message , 2 as timer_interval_second
		RETURN 
	END
	...
	END
		...
		ELSE
		BEGIN
			SELECT '0' as response_code, 'Daily limit reached' as response_message , 2 as timer_interval_second
		RETURN 
...
ELSE
BEGIN
	SELECT '0' as response_code, 'Please update your mobile number' as response_message , 2 as timer_interval_second
...
ELSE
BEGIN
	SELECT '2' as response_code, 'Multiple Logins Not Allowed' as response_message , 2 as timer_interval_second
