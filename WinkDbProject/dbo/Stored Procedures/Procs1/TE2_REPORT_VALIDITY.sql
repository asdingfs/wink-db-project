CREATE PROC [dbo].[TE2_REPORT_VALIDITY]
(
...
BEGIN
	...
	IF(@isResident is null or @isResident='')
		SET @isResident = NULL
	...
	BEGIN
		...
		(--1 START
			...
			(
				-----table1
				SELECT * from
				(SELECT customer_id,0 as points,location as GPS_location,ip_address,created_at, '' as Q1, '' as Q2
			...
			WHERE ( @validity is null 
				...
				AND
				( @isResident is null 
					or (@isResident = 'Yes' AND T.Q1 = 'Yes')  
					or (@isResident = 'No' AND T.Q1 = 'No')
				)
				AND
				( @names is null 
					or 
					(	@names not like 'Others' 
						AND T.Q2 like @names+'%'
					)  
					or (@names like 'Others' 
						AND T.Q2 not like 'Chill%'
						AND T.Q2 not like 'Zen%'
						AND T.Q2 not like 'Stellar%')
				)
		) AS TEMP----1 END
