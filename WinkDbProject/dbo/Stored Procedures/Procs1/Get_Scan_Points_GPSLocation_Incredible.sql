CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_Incredible]          
...
Declare @cobrandcanid varchar(50)
...
set @global_campaign = 5
set @global_campaign = (Select top 1 campaign_id from campaign where cast(campaign_end_date as date) >=  cast(@current_datetime as date) and campaign_id !=@global_campaign and merchant_id != 248 and campaign.campaign_status='enable' order by campaign_id desc)
set @default_image_id = (select id from campaign_small_image where campaign_id = @global_campaign and small_image_status=1)


If CAST(@CURRENT_DATETIME as time) > '13:00:00' AND CAST(@CURRENT_DATETIME as time) <= '20:00:00'

BEGIN
		    
-- Start Safeguard
--1. Begin check time between 00:30 and 05:30
--User will not be able to scan from midnight to 5:30 in the morning  
...
if(CAST(@CURRENT_DATETIME AS DATE) > CAST('2018-07-08' AS DATE))
BEGIN

SET @RETURN_NO='301' -- scan time is too frequent                           
GOTO Err

END

if ((SELECT count(*) from customer_earned_points 
where qr_code = 'DisneyIncredibles2_DisneyIncredibles2_01_33942' ) >= 500)
BEGIN

SET @RETURN_NO='303' -- scan time is too frequent                           
GOTO Err

END


if ((SELECT count(*) from customer_earned_points where customer_id = @CUSTOMER_ID AND qr_code = 'DisneyIncredibles2_DisneyIncredibles2_01_33942' ) > 0)
BEGIN
...
--if((select CAST(created_at AS DATE) from customer where customer_id = @CUSTOMER_ID) >= CAST('2018-06-20' AS DATE) and
--(select CAST(created_at AS DATE) from customer where customer_id = @CUSTOMER_ID) <= CAST('2018-07-31' AS DATE)
--)
--BEGIN
...
--SET @RETURN_NO='903' -- scan time is too frequent                           
--GOTO Err
...
--Err
	...
	BEGIN  
		SELECT '1' as response_code,'Congrats! You have received 50 WINK+ points which will be credited to your WINK+ account. You can view your points under Account > My Rewards.' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
		, 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='304' -- Old Citi Corbrand Card
	BEGIN  
		SELECT '0' as response_code, 'Sorry! You’ve already linked a Citibank SMRT Card to your WINK+ account.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='303' -- Not Citi Corbrand Card 
	BEGIN  
		SELECT '0' as response_code, 'The WINK+ x Disney•Pixar Incredibles 2 Promotion has ended. Look out for more giveaways coming soon!' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='302' -- Already participated 
	BEGIN  
		SELECT '0' as response_code, 'You have already participated in the WINK+ & Disney•Pixar Incredibles 2 Promotion.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='600' -- Over Limited Per Month 
	BEGIN  
		SELECT '0' as response_code, 'We noticed that you have already scanned this QR code.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='309' -- Over Limited Per Month 
	BEGIN  
		SELECT '0' as response_code, 'WINK+ points have been claimed by another WINK+ account using the same Citibank SMRT Card CAN ID.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	--ELSE IF @RETURN_NO='309' -- Over Limited Per Month 
	--BEGIN  
		...
		--RETURN 
	--END
	ELSE IF @RETURN_NO='301' -- Campaign ended
	BEGIN  
		SELECT '0' as response_code, 'Scanning of this QR Code is available during Citibank roadshows only.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='903' -- Campaign ended
	BEGIN  
		SELECT '0' as response_code, 'You’re already a WINK+ user. Look out for more promotions happening soon!' as response_message , 3 as timer_interval_second
		RETURN 
	END
