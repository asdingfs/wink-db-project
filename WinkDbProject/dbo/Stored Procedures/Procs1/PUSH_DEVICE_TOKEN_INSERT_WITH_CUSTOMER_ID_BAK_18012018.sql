Create PROC [dbo].[PUSH_DEVICE_TOKEN_INSERT_WITH_CUSTOMER_ID_BAK_18012018]
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				UPDATE push_device_token set WID = @WID, updated_at = (SELECT TODAY FROM VW_CURRENT_SG_TIME), active_status = @active_status, customer_id = @customer_id
		...
		BEGIN
			...
			BEGIN
				...
				SET @primary_customer_id = (SELECT customer_id FROM push_device_token WHERE customer_id IS NOT NULL AND device_token = @device_token)
				...
				BEGIN
					...
					BEGIN
						...
						VALUES (@primary_customer_id,'disable',(select today from VW_CURRENT_SG_TIME),'Token M',@device_token,@device_id,@reason_primary)

					END

					--1)Lock multiple account
					...
					INSERT INTO System_Log (customer_id,action_status,created_at,reason,device_token,device_type,locked_desc)
					VALUES (@customer_id,'disable',(select today from VW_CURRENT_SG_TIME),'Token M',@device_token,@device_id,@reason)
		...
		--EXEC PUSH_LOCK_MULTIPLE_ACCOUNT_ON_SAME_DEVICE @device_token,@device_id,@customer_id,@active_status
