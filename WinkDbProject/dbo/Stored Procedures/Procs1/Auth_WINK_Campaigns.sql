CREATE PROCEDURE [dbo].[Auth_WINK_Campaigns] 
(@auth varchar(150))
AS
BEGIN
   ...
   --SMRT35thAnniversaryPhase7--
	...
	where campaign_id = @GreenLivingId;
	...
	where campaign_id = @BusShelterTenderId;
	...
	BEGIN
		...
		(
            ...
            --and d.campaign_id != @donationCampaignId

			--union 
			---- ST Pocket Money Fund Donation
			--SELECT w.campaign_id FROM winktag_campaign as w WHERE w.survey_type !='merchant' and  w.WINKTAG_STATUS like '1'
			--AND CONVERT(datetime,@CURRENT_DATETIME) >= CONVERT(datetime,from_date)
			--AND CONVERT(datetime,@CURRENT_DATETIME) <= CONVERT(datetime,to_date)  
			--AND campaign_id = @donationCampaignId
			--AND @donationQualified = 1
