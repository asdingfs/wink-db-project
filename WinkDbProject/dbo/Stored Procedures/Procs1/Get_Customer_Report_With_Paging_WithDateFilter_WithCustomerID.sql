CREATE PROCEDURE [dbo].[Get_Customer_Report_With_Paging_WithDateFilter_WithCustomerID]      
 (@start_date datetime,      
  @end_date datetime,      
  @customer_id INT,
  @group_id varchar(10),
			...
			BEGIN
				...
				(
					...
					Group by customer_earned_points.customer_id 
						...
						( Select customer_id,SUM(ISNULL(customer_balance.total_winks,0))AS alltime_Total_Winks 
														
						from customer_balance       
						where customer_id = @customer_id --and 
						--( CAST (customer_earned_winks.created_at AS date) <= CAST (@end_date AS date) )      
						GROUP By customer_balance.customer_id   
						),
						----------------------------------
