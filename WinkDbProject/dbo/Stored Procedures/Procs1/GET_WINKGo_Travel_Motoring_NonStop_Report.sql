...
CREATE Procedure [dbo].[GET_WINKGo_Travel_Motoring_NonStop_Report]
(
  @from_date datetime,
  @to_date datetime,
...
print ('AA')
	...
	A.trans_amount,A.updated_at,A.gps_location,A.ip_address,A.points_expired_status,D.WID, can_id from nonstop_net_canid_earned_points AS A ,customer as D 
	 where
	 /*(
	...
	---- Point Expired is NULL 
			...
			select ROW_NUMBER() OVER(ORDER BY A.updated_at ASC) AS row_no, A.business_date,A.can_id,B.card_type,C.email,A.customer_id,A.total_points,A.gps_location,A.ip_address,A.trans_amount,
			...
			and ( (@tran_type is null  and A.card_type!='10') or B.card_code = @tran_type)
