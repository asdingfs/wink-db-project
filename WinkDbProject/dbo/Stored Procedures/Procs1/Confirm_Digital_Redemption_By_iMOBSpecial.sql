CREATE Procedure [dbo].[Confirm_Digital_Redemption_By_iMOBSpecial]
(
...
Declare @total_purchase int
        ...
        IF EXISTS (select 1 from iMOBSpecial where customer_id = @customer_id and event_name =@event_name)
        BEGIN
          IF Exists (select 1 from wink_digital_redemption  where NRIC =@NRIC and @NRIC!='' and event_name =@event_name)
						...
						set @redeem_customer_id = (select distinct customer_id from wink_digital_redemption where 
						event_name =@event_name and @NRIC =NRIC)
