...
Create PROC [dbo].[Get_Scan_Points_GPSLocation_bak]          
...
DECLARE @Valid int 
/*If (CAST(@CURRENT_DATETIME as date)> CAST ('2016-11-05' as date))
Begin

SELECT '0' as response_code, 'Please download WINK+ new version' as response_message , 3 as timer_interval_second
...
BEGIN
	If CAST(@CURRENT_DATETIME as time) < '04:30:00' 
  ...
  BEGIN --- Begin check time between 00:30 and 05:30
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO CUSTOMER_EARNED_POINTS (customer_id,campaign_booking_id,campaign_id,points,last_scanned_time,qr_code,created_at,GPS_location,ip_address) VALUES
					(@CUSTOMER_ID,@BOOKING_ID,@CAMPAIGN_ID,@SCAN_VALUE,@CURRENT_DATETIME,@qrcode,@CURRENT_DATETIME,@GPS_location,@ip_address)
					IF(@@ROWCOUNT>0)
					BEGIN
	...
	BEGIN
		SET @RETURN_NO='006'   -- customer does not exist                        
		GOTO Err
	END
	...
	BEGIN  
		SELECT '0' as response_code, 'Scan interval per panel : 24 hours Scan interval per panel : 24 hours Scan interval per panel : 24 hours Scan interval per panel : 24 hours' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='003' 
	BEGIN  
		SELECT '0' as response_code, 'Invalid QR Code' as response_message , 3 as timer_interval_second
		--SELECT '0' as response_code, 'Invalid QR Code Invalid QR Code Invalid QR Code Invalid QR Code Invalid QR Code ' as response_message , 3 as timer_interval_second
		RETURN 
	END
 ...
 ELSE 
	BEGIN
	SELECT '0' as response_code, 'Invalid Scan' as response_message , 3 as timer_interval_second
	--SELECT '0' as response_code, 'Invalid Scan Invalid Scan Invalid Scan Invalid Scan Invalid Scan Invalid Scan Invalid Scan Invalid Scan Invalid Scan' as response_message , 3 as timer_interval_second
		RETURN 
	END
