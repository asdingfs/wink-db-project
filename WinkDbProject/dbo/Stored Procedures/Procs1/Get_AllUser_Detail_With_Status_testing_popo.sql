...
CREATE PROCEDURE [dbo].[Get_AllUser_Detail_With_Status_testing_popo]
	 (@email varchar(150),
	  @name varchar (200),
	  @status varchar(20),
	...
	If (@created_from IS NOT NULL AND @created_from !='' AND @created_to IS NOT NULL AND @created_to !='')
	 ...
	 BEGIN
				...
				ELSE
					...
					customer.customer_id = customer_action_log_temp.customer_id and rn = 1 and Lower(customer.first_name +' '+ customer.last_name) LIKE '%'+Lower(@name)+'%' AND Lower(customer.email) LIKE '%'+@email+'%' 
