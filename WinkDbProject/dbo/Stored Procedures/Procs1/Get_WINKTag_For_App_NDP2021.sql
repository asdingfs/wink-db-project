CREATE PROCEDURE [dbo].[Get_WINKTag_For_App_NDP2021] 
	(@auth varchar(150)
	 )
AS
BEGIN
...
Declare @GiveWINKaNameScanned int = 0
Declare @GiveWINKaNameAnswered int = 0
...
DECLARE @GiveMEaNameQualified int

set @GiveWINKaNameScanned = ISNULL( (select count(*) FROM CUSTOMER_EARNED_POINTS WHERE CUSTOMER_ID = @CUSTOMER_ID AND (
 QR_CODE like 'GWAN_NAME%' )
and created_at>'2021-04-21'),0) 

IF (@GiveWINKaNameScanned>0)
	set @GiveWINKaNameAnswered = ISNULL( (select count(*) FROM winktag_customer_earned_points  WHERE CUSTOMER_ID = @CUSTOMER_ID 
	AND campaign_id=163 and created_at>'2021-04-21'),0) 

set @GiveMEaNameQualified=  @GiveWINKaNameScanned - @GiveWINKaNameAnswered;
	...
	Begin
            ...
            ;With active_campaign as (     
			  ...
			  --       -- Deepavali
					...
					union 
					-- GiveMEaNameCampaign
					SELECT * FROM winktag_campaign as w WHERE w.survey_type !='merchant' and  w.internal_testing_status = 1
					AND CONVERT(datetime,@CURRENT_DATETIME) >= CONVERT(datetime,from_date)
					AND CONVERT(datetime,@CURRENT_DATETIME) <= CONVERT(datetime,to_date)  
					And (w.campaign_id = 167)
					AND @totalNDP2021Size < 80
			...
			-- if (@GiveMEaNameQualified>0)
			--	set  @total_active_campaign = @total_active_campaign - (@GiveMEaNameQualified - 1 )
			...
			ELSE IF EXISTS (select 1 from (Select * from [winkwink].[dbo].winktag_campaign  as  d
				 ...
				 d.winktag_status = 1    AND CONVERT(datetime,@CURRENT_DATETIME) >= CONVERT(datetime,from_date)
					 ...
					 --or campaign_id = 101 
					 or campaign_id = 116 or campaign_id = 128))
			BEGIN
				;With active_campaign as (     
