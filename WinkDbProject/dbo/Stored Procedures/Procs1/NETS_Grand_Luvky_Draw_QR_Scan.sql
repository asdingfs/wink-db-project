CREATE PROC [dbo].[NETS_Grand_Luvky_Draw_QR_Scan]          
...
-- NETS Grand Lucky Draw
DECLARE @user_creation_time datetime



If CAST(@CURRENT_DATETIME as datetime) >= '2018-09-21 10:30:00' AND CAST(@CURRENT_DATETIME as datetime) <= '2018-09-21 18:30:00'

BEGIN
	
	Select @CUSTOMER_ID =customer_id, @user_creation_time = created_at from customer where customer.auth_token = @customer_tokenid and status ='enable' 
		
	-- account must be created after 10:30
	IF(CAST(@user_creation_time AS DATETIME) < CAST('2018-09-21 10:30:00' AS DATETIME))
		BEGIN

			SET @RETURN_NO='301' -- 50 bonus WINK+ points are for new users only.              
			GOTO Err

		END

	IF ((SELECT count(*) from customer_earned_points where customer_id = @CUSTOMER_ID AND qr_code = '0923_NETSDraw_02_33946' ) > 0)
		BEGIN

			SET @RETURN_NO='302' -- You have already participated in the WINK+ x NETS Grand Prize Giveaway.                       
			GOTO Err

		END

		-- need to be updated to >=200 
	IF ((SELECT count(*) from customer_earned_points where qr_code = '0923_NETSDraw_02_33946' ) >= 200)
		BEGIN
			SET @RETURN_NO='303' -- Sorry! This promotion has ended.                          
			GOTO Err
		END
    ...
    and CAST(customer_earned_points.created_at as Date) = CAST(@CURRENT_DATETIME as Date))>=250
   		BEGIN
	...
	Group by CAST (created_at as DATE))>10)
	BEGIN
	
		IF ( (select DATEDIFF(second,MIN(created_at),MAX(created_at)) from (select top 10 * from customer_earned_points 
		...
		(Select customer.customer_id from customer where customer.auth_token = @customer_tokenid)
			...
			order by earned_points_id desc)a )<=30)
		...
		BEGIN
			...
			BEGIN
				...
				Select customer.customer_id,'disable',@CURRENT_DATETIME,'Script Scanning'
	...
	BEGIN
	   ...
	   IF ((select COUNT(*)+1 from customer_earned_points where customer_earned_points.customer_id =   
	   (Select customer.customer_id from customer where customer.auth_token = @customer_tokenid)
	...
	group by c.customer_id,c.GPS_location having COUNT(*)>=25)
	...
	BEGIN
		...
		BEGIN
			Insert into System_Log (customer_id, action_status,created_at,reason)
			Select customer.customer_id, 'disable',@CURRENT_DATETIME,'LBS' from customer where customer.auth_token = @customer_tokenid
		END
		SET @RETURN_NO='001' -- scan time is too frequent                           
		GOTO Err
	END
	...
	BEGIN
		...
		BEGIN
			Insert into System_Log (customer_id, action_status,created_at,reason)
			Select customer.customer_id,'disable',@CURRENT_DATETIME,'52.74.3.233' from customer where customer.auth_token = @customer_tokenid
		END
		SET @RETURN_NO='001' -- scan time is too frequent                           
		GOTO Err
	END
...
-- End Safeguard----------------------------------------------------------------------------
	...
	BEGIN  -- 4.. Check customer is enable
		...
		END
		ELSE IF EXISTS (SELECT 1 FROM asset_type_management as s WHERE  s.qr_code_value = @qrcode
		...
		ELSE IF EXISTS (SELECT 1 FROM asset_type_management as s WHERE  s.qr_code_value = @qrcode
		AND s.asset_status = '1'  AND Lower(s.special_campaign) ='yes'
		AND (CONVERT(CHAR(10),@CURRENT_DATETIME,111) 
	    ...
	    SET @last_station_qrCode = (Select Top 1 customer_earned_points_log.qr_code from customer_earned_points_log where customer_earned_points_log.customer_id = @CUSTOMER_ID order by customer_earned_points_log.earned_points_id desc)
			...
			BEGIN
				...
				BEGIN
					...
					ELSE	
					BEGIN 
						SET @RETURN_NO='004' -- Insert Failed                         
						GOTO Err
					END
				END
			END
	...
	BEGIN  
		...
		RETURN 
	END
	ELSE IF @RETURN_NO='303' -- Reached 200 participants
	BEGIN  
		SELECT '0' as response_code, 'Sorry! This promotion has ended.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='302' -- Already participated 
	BEGIN  
		SELECT '0' as response_code, 'You have already participated in the WINK+ x NETS Grand Prize Giveaway.' as response_message , 3 as timer_interval_second
		RETURN 
	END

	ELSE IF @RETURN_NO='301' -- Only for new users
	BEGIN  
		SELECT '0' as response_code, '50 bonus WINK+ points are for new users only.' as response_message , 3 as timer_interval_second
		RETURN 
	END
