CREATE PROC [dbo].[SPG_UOB_QR_Scan]          
...
-- this small logo image is only for SimplyGo UOB Mastercard QR campaign
set @default_image_id =303
...
-- End Safeguard----------------------------------------------------------------------------
	...
	BEGIN  -- 4.. Check customer is enable
		...
		AND s.asset_status = '1' 
		AND CAST(@CURRENT_DATETIME AS datetime)between '2019-05-27 05:30:00.000' AND '2019-09-04 23:59:59.000'
		)
			...
			IF(
				...
				BEGIN
					...
					BEGIN
						...
						SET @RETURN_NO='010' -- 500 points                        
						GOTO Err
					END
					ELSE
					BEGIN
						SET @RETURN_NO='000' -- 1 point                        
						GOTO Err
					END
			...
			ELSE
				BEGIN
				 -- there are already 1000 scans for today 
					SET @RETURN_NO='009' -- Max 1,000 scans reached           
					GOTO Err
				END
	...
	BEGIN  
		SELECT '0' as response_code, 'Oops! You can only scan this QR Code once per day. Try again tomorrow!' as response_message , 3 as timer_interval_second,@SMALL_IMAGE_URL AS small_banner_url,@SMALL_WEBSITE_URL AS small_website_url
		RETURN 
	END

	ELSE IF @RETURN_NO='009' -- Max 1,000 scans given out already
	BEGIN  
		SELECT '0' as response_code, 'All WINK+ points have been given out for the day. Try again tomorrow!' as response_message , 3 as timer_interval_second,@SMALL_IMAGE_URL AS small_banner_url,@SMALL_WEBSITE_URL AS small_website_url
		RETURN 
	END
	ELSE IF @RETURN_NO='010' -- Congrats Message for successful scanning (500 WINK+ points)
	...
	IF(@SCAN_VALUE = 500)
	 ...
	 END
	 SELECT '2' as response_code,'Congrats! It’s your lucky day! You’ve earned 500 WINK+ points credited directly to your WINK+ account.!' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
		, 3 as timer_interval_second,@default_image_id as image_id
		RETURN 
	END
	ELSE IF @RETURN_NO='000' -- Congrats Message for successful scanning (1 WINK+ point)

	BEGIN 
	 SELECT '1' as response_code,'Congrats! You’ve earned 1 WINK+ point. Tip: Earn points daily by scanning WINK+ QR Codes in SMRT stations.' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
		, 3 as timer_interval_second,@default_image_id as image_id
		RETURN 
	END
