...
CREATE PROCEDURE [dbo].[Get_QRCodeScan_Orchard_Citibankcard]
(
	@wid varchar(50),
	@customerId int,
	@start_date datetime,
...
BEGIN
	...
	SET @cusid = (SELECT Top 1 customer.customer_id FROM customer WHERE customer.email like '%'+ @email + '%');
 	...
 	SELECT * FROM 
	(
		SELECT orchard_citibank.points As total_scan ,orchard_citibank.qr_code 
		,orchard_citibank.corbrand_card
		,orchard_citibank.registered_date_for_corbrand_card
		,orchard_citibank.GPS_location
		,customer.[status] as customer_status
		,orchard_citibank.ip_address
	   ...
	   ,(CONVERT(int,CONVERT(char(8),@current_datetime,112))-CONVERT(char(8),Cast(customer.date_of_birth as date),112))/10000 AS age
		...
		orchard_citibank.created_at as scan_time 

		,datepart(HOUR,orchard_citibank.created_at)as created_hour
		,orchard_citibank.customer_id as customer_id

		,ROW_NUMBER() OVER(order by
		orchard_citibank.customer_id desc,
		orchard_citibank.created_at 
		desc) as id,
    
		COUNT(*) OVER()  as total_count

		from orchard_citibank
		JOIN customer

		ON orchard_citibank.customer_id = customer.customer_id

		JOIN asset_type_management

		ON orchard_citibank.qr_code = asset_type_management.qr_code_value

		WHERE  (@wid is null or customer.WID like '%'+@wid+'%')
		AND (@customerId is null or orchard_citibank.customer_id = @customerId)
		AND (@start_date IS NULL OR CAST(orchard_citibank.created_at as Date) BETWEEN @start_date AND @end_date) 
	
		AND (@qr_code IS NULL OR orchard_citibank.qr_code like '%' + @qr_code + '%') 

		AND (@qr_ip_address IS NULL OR orchard_citibank.ip_address like '%' + @qr_ip_address + '%')
		...
		AND (@location IS NULL OR orchard_citibank.GPS_location like '%' + @location + '%')
