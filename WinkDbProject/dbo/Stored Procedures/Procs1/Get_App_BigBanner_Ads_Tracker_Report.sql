CREATE PROCEDURE [dbo].[Get_App_BigBanner_Ads_Tracker_Report]
(
  @url_name varchar(50),
...
from largebanner_ads_tracker as t
...
where (@url_name is null or img.large_image_name like '%'+@url_name+'%')
...
from 
(select * from largebanner_ads_tracker where CAST(created_at as date) BETWEEN 
CAST(@from_date as date) and CAST(@to_date as date)
and (@ip_address is null or largebanner_ads_tracker.ip_address like '%'+@ip_address+'%')
