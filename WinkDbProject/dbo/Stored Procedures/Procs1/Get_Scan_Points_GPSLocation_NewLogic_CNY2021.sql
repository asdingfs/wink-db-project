CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_NewLogic_CNY2021]          
...
DECLARE @CNY_CAMPAIGN_START_TIME DATETIME
DECLARE @CNY_CAMPAIGN_END_TIME DATETIME
DECLARE @CNY_CAMPAIGN_END_TIME_STAGE1 DATETIME
DECLARE @CNY_CAMPAIGN_STAGE1_MAX_POINTS INT
DECLARE @CNY_CAMPAIGN_TOTAL_MAX_POINTS INT
DECLARE @time_stage INT
...
--- to be check for CNY Campaign.

set @default_image_id =400
...
BEGIN
 ...
 set @CNY_CAMPAIGN_START_TIME = '2021-02-01 00:00:00.000'  --'2021-02-12 00:00:00.000'
 set @CNY_CAMPAIGN_END_TIME_STAGE1 ='2021-02-03 00:00:00.000'	--'2021-02-15 00:00:00.000'
 set @CNY_CAMPAIGN_END_TIME = '2021-02-26 23:59:59.000'
 set @CNY_CAMPAIGN_STAGE1_MAX_POINTS  = 153600 
 set @CNY_CAMPAIGN_TOTAL_MAX_POINTS =  384000 
 set @time_stage = 0
...
If CAST(@CURRENT_DATETIME as time) > '00:00:00' 
  ...
  --1.1.  Check the Campaign start and end time
ELSE IF (@CURRENT_DATETIME < @CNY_CAMPAIGN_START_TIME) or (@CURRENT_DATETIME > @CNY_CAMPAIGN_END_TIME)
 BEGIN
		SELECT '0' as response_code, 'Invalid Scan' as response_message , 3 as timer_interval_second
			RETURN 
END

  --1.2 Check first stage inventory
ELSE IF (@CURRENT_DATETIME >= @CNY_CAMPAIGN_START_TIME and @CURRENT_DATETIME < @CNY_CAMPAIGN_END_TIME_STAGE1)
BEGIN
	set @time_stage = 1
	IF (Select SUM(points) from customer_earned_points where qr_code like '%WCNY_Bonus_01_34002%'
	     and (created_at>=@CNY_CAMPAIGN_START_TIME)
		 and (created_at<@CNY_CAMPAIGN_END_TIME_STAGE1) 
    	 
     )>=@CNY_CAMPAIGN_STAGE1_MAX_POINTS 
	 BEGIN
		SELECT '0' as response_code, 'Invalid Scan' as response_message , 3 as timer_interval_second
			RETURN 
	 END
...
--1.3 Check second stage inventory
ELSE IF (@CURRENT_DATETIME >=@CNY_CAMPAIGN_END_TIME_STAGE1 and @CURRENT_DATETIME <= @CNY_CAMPAIGN_END_TIME)
BEGIN
	set @time_stage = 2
	IF (Select SUM(points) from customer_earned_points where qr_code like '%WCNY_Bonus_01_34002%'
	     and (created_at>=@CNY_CAMPAIGN_START_TIME)
		 and (created_at<=@CNY_CAMPAIGN_END_TIME) 
    	 
     )>=@CNY_CAMPAIGN_TOTAL_MAX_POINTS 
	 BEGIN
		SELECT '0' as response_code, 'Invalid Scan' as response_message , 3 as timer_interval_second
			RETURN 
	 END
...
-- End Safeguard----------------------------------------------------------------------------
	...
	if (@time_stage = 2 and @SCAN_VALUE = 8)
			set @SCAN_VALUE = 3
	...
	BEGIN  
		...
		SELECT '0' as response_code, 'Oops! You have already scanned this QR Code.' as response_message , 3 as timer_interval_second
		RETURN 
	END
