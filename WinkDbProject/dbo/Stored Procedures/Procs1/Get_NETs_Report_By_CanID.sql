...
CREATE PROCEDURE [dbo].[Get_NETs_Report_By_CanID]
(
...
INSERT INTO @TEMP_NETS_REPORT
				select MAX(p.customer_id),p.can_id, sum(total_points) as total_points, sum(total_tabs) as total_tabs, MAX(created_at)
...
SELECT P.customer_id,P.can_id,([customer].first_name + ' ' + [customer].last_name) as customer_name, [customer].email
