...
CREATE Procedure [dbo].[GET_WFH_Motoring_NonStop_Report]
(
  @from_date datetime,
  @to_date datetime,
...
print ('AA')
	...
	A.trans_amount,A.updated_at,A.gps_location,A.points_expired_status,D.WID as can_id from nonstop_net_canid_earned_points AS A ,customer as D  
	 where
	 /*(
	 A.created_at is not null or A.created_at !='') 
	 and */
 
	 (@can_id is null or D.WID like @can_id +'%')
...
;WITH nonstop AS 
 ...
 where  cast (A.business_date as date) >=  cast (@from_date as date)
	...
	and (@can_id is null or D.WID like @can_id +'%')
	and (@customer_id is null or A.customer_id = @customer_id )
	and (@points_credit_status is null or A.points_credit_status = @points_credit_status)
	--and (@points_expired_status is null or A.points_expired_status = @points_expired_status)
	and 
