...
CREATE PROCEDURE [dbo].[Get_QRCodeScan_Report_With_Paging_v05]
(
	@customer_id int,
	@wid varchar(50),
	@start_date datetime,
...
BEGIN
	...
	SELECT customer_earned_points.points As total_scan ,customer_earned_points.qr_code 
   ...
   ,(CONVERT(int,CONVERT(char(8),@current_datetime,112))-CONVERT(char(8),Cast(customer.date_of_birth as date),112))/10000 AS age
	...
	asset_type_management.asset_name
		...
		select * from w	  where w.id between @intStartRow and @intEndRow
		order by w.scan_time desc
END
ELSE -- FOR Export, no need use temproray table.
BEGIN
