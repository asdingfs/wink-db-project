CREATE PROCEDURE [dbo].[Get_Customer_Report_With_Paging_WithDateFilter_Locked_backup]      
 (@start_date datetime,      
  @end_date datetime,      
		...
		BEGIN
			...
			BEGIN
				...
				(
					...
					final.ip_scanned,final.total_trips_, final.total_wink_go_,
					ROW_NUMBER() OVER(ORDER BY isnull(total_scans_,0) DESC) as intRow,
					...
					AND (@status is null  or Lower(customer.[status]) LIKE Lower(@status+'%'))
							...
							Group by customer_id
							   ...
							   Group By final.customer_id,final.first_name,final.last_name ,final.email,final.[status],total_scans_,final.ip_address,final.ip_scanned,final.total_trips_,total_wink_go_,final.WID,final.[group],
							   total_count
								),  
