CREATE PROCEDURE [dbo].[Get_All_Asm_And_AsmBooking_ByAssetName_And_Station]
	(
...
BEGIN
	...
	where 
	Lower(asset_type_management.asset_name)LIKE Lower('%'+@asset_name+'%') AND
	Lower(station.station_name)LIKE Lower('%'+@station_name+'%')
