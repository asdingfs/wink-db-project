CREATE PROC [dbo].[Robin_Hood_QR_Scan]          
...
If CAST(@CURRENT_DATETIME as datetime) >= '2018-11-26 16:45:00' AND CAST(@CURRENT_DATETIME as datetime) <= '2018-11-27 20:00:00'
...
BEGIN
	...
	-- account must be created after 17:45
	IF(CAST(@user_creation_time AS DATETIME) < CAST('2018-11-26 16:45:00' AS DATETIME))
		BEGIN

			SET @RETURN_NO='301' -- 10 bonus WINK+ points are for new users only.              
			GOTO Err

		END

	IF ((SELECT count(*) from customer_earned_points where customer_id = @CUSTOMER_ID AND qr_code = '1127_RobinHood_02_33948' ) > 0)
		BEGIN

			SET @RETURN_NO='302' -- You have already participated in the Robin Hood campaign.                       
			GOTO Err

		END

		-- need to be updated to >=500 
	IF ((SELECT count(*) from customer_earned_points where qr_code = '1127_RobinHood_02_33948' ) >= 2)
		BEGIN
...
-- End Safeguard----------------------------------------------------------------------------
	...
	BEGIN  
		SELECT '1' as response_code,'Congrats! You have received 10 WINK+ points which will be credited to your WINK+ account. You can view your points under Account > My Rewards.' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
		, 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='303' -- Reached 500 participants
	BEGIN  
	...
	BEGIN  
		SELECT '0' as response_code, 'You have already participated in the Robin Hood campaign.' as response_message , 3 as timer_interval_second
		RETURN 
	END

	ELSE IF @RETURN_NO='301' -- Only for new users
	BEGIN  
		SELECT '0' as response_code, '10 bonus WINK+ points are for new users only.' as response_message , 3 as timer_interval_second
		RETURN 
	END
