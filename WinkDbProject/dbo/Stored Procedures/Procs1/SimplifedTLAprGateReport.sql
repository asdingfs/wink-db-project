...
CREATE PROCEDURE [dbo].[SimplifedTLAprGateReport]
(
	@wid varchar(50),
	@customer_id int,
	@name varchar(100),
...
BEGIN
	...
	(
		...
		a.gate_id as gateId, 
		winktag.answer as referralCode,
		...
		ON e.assetId = a.id
		LEFT JOIN winktag_customer_survey_answer_detail as winktag
		ON winktag.customer_id = e.customer_id
		AND winktag.campaign_id = 160
		JOIN wink_gate_booking as b
		ON e.bookingId = b.id
		WHERE b.wink_gate_campaign_id = 41
		AND b.[status] = 1
		AND (@bankType is null or winktag.answer like @bankType+'%')
