CREATE PROC [dbo].[GBB_NewUser_QR_Scan]          
...
-- this small logo image is only for garden bb
set @default_image_id =306

SET @between_start_date = '2019-10-20 22:00:00.000'
SET @between_end_date = '2019-10-25 17:00:00.000'
SET @qrcode_start_time = '13:00:00.000'
SET @qrcode_end_time = '22:00:00.000'

SET @check_time = Cast(@CURRENT_DATETIME as time)
print('Current Time')
print(@check_time)
...
-- 1. Check customer is enable
	BEGIN  
-- 1 check if it's within the specific time frame   
--  check new user or existing user
		SELECT @customer_created_date = created_at FROM [winkwink].[dbo].customer 
		where customer_id = @CUSTOMER_ID

		print('Customer Created Date')
		...
		IF EXISTS (SELECT 1 FROM asset_type_management as s
			...
			AND(
			Cast(@customer_created_date as date) >= s.scan_start_date
			AND
			Cast(@customer_created_date as date) <= s.scan_end_date
			)
			)   
...
-- 4 Valid QR code from 5pm to 10pm
				IF (Cast(@CURRENT_DATETIME as time) < @qrcode_start_time
					  ...
					  BEGIN
						 SET @RETURN_NO='010'   -- INVALID QR CODE  time                      
						 GOTO Err
					  END
---5 check customer created date between date and limit time from monday(21) to thursday(24)


SET @check_date = Cast(@customer_created_date as date)
print('Customer date')
print(@check_date)

print('Between start date')
Set @between_start_date =  Cast(@between_start_date as date)
print(@between_start_date)

Set @between_start_date = '2019-09-09 22:00:00.000'
Set @between_end_date = '2019-09-11 22:00:00.000'

				IF (Cast(@customer_created_date as date) > Cast(@between_start_date as date) 
					   AND 
					 Cast(@customer_created_date as date)< Cast(@between_end_date as date)
					) 
					  BEGIN
						 SET @RETURN_NO='014'   -- INVALID QR CODE                        
						 GOTO Err
					  END

				
-- 6 inventory count start

				IF(
					(SELECT COUNT(*) from customer_earned_points 
					where qr_code like 'GBB_New_User_01%'
					) 
					< 2000 )
...
---- already full
				ELSE
					BEGIN
					 -- there are already 2000 scans for today 
						SET @RETURN_NO='009' -- Max 2,000 points given out already           
						GOTO Err
					END
...
ELSE -- Customer disable
	...
	BEGIN  
		...
		RETURN 
	END
	ELSE IF @RETURN_NO='009' -- fully redeemed
		BEGIN  
			...
			RETURN 
		END
	ELSE IF @RETURN_NO='010' -- 5pm to 10pm
		BEGIN  
	...
	ELSE IF @RETURN_NO='013' -- Monday to Thurday
		BEGIN  
			...
			RETURN 
		END
	ELSE IF @RETURN_NO='014' -- Monday to Thurday
		BEGIN  
			SELECT '0' as response_code, 'Invalid QR Scan Customer Created Date.' as response_message , 3 as timer_interval_second,@SMALL_IMAGE_URL AS small_banner_url,@SMALL_WEBSITE_URL AS small_website_url
			RETURN 
		END
	ELSE IF @RETURN_NO='000' -- Congrats Message for successful scanning
	BEGIN 
	 SELECT '1' as response_code,'Congrats! You have won 100 WINK+ point! Scan more WINK+ QR codes to keep winning.' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
