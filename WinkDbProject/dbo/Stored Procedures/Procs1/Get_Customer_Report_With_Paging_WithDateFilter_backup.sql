CREATE PROCEDURE [dbo].[Get_Customer_Report_With_Paging_WithDateFilter_backup]      
 (@start_date datetime,      
  @end_date datetime,      
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				;WITH customer_temp AS
				(
					select a.customer_id,a.WID, a.first_name,a.last_name ,a.email,a.[status],a.[group],total_scans_,ip_address,ip_scanned,total_trips_, total_wink_go_,
				...
				Group by customer_id
					...
					Group By a.customer_id,a.WID,a.first_name,a.last_name ,a.email,a.[status],a.[group],total_scans_,ip_address,ip_scanned,total_trips_, total_wink_go_
			...
			BEGIN
				...
				Group by customer_id
					...
					Group By a.customer_id,a.WID, a.first_name,a.last_name ,a.email,a.status,[group],total_scans_,total_winkplays_,ip_address,ip_scanned,total_trips_, total_wink_go_
					Having ISNULL(total_trips_,0)+ ISNULL(total_scans_,0) + ISNULL(total_wink_go_,0) + ISNULL(total_winkplays_, 0)>0
					) AS final
					join 
					customer on 
					final.customer_id = final.customer_id
					where intRow between @intStartRow and @intEndRow
					Group By final.customer_id,final.WID, final.first_name,final.last_name ,final.email,final.status,final.[group],total_scans_,final.total_winkplays_, final.ip_address,final.ip_scanned,final.total_trips_,
					final.total_wink_go_,
					total_count
					Having ISNULL(total_trips_,0)+ ISNULL(total_scans_,0) + ISNULL(total_wink_go_,0) + ISNULL(total_winkplays_,0)>0
