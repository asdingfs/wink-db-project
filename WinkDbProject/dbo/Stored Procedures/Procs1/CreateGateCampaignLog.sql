CREATE PROCEDURE  [dbo].[CreateGateCampaignLog] 
(
	@campaign_id int,
	@wink_gate_campaign_id int,
	@total_points int,
	@status int,
...
BEGIN 
	...
	BEGIN TRY
		...
		BEGIN
			DECLARE @actionTableName varchar(100) = 'gate_campaign_log';
			-- Get Log ID
			Set @log_id = 
			...
			SELECT @campaignName =campaign_name, @startDate = campaign_start_date, @endDate = campaign_end_date
			FROM campaign
			...
			Begin
				-- SET Link_url 
				Set @link_url = 'adminactiondetail/gatecampaignedit';
				-- Add Action 
				...
				INSERT INTO [dbo].[gate_campaign_old_data_log]
						   ([action_id]
				...
				-- Add New Data Log 
				INSERT INTO [dbo].[gate_campaign_new_data_log]
				   ([action_id]
				   ,[wink_gate_campaign_id]
				   ,[campaign_name]
				   ,[new_total_points]
				...
				where id = @wink_gate_campaign_id;
			...
			BEGIN 
				-- SET Link_url 
				SET @link_url = 'adminactiondetail/gatecampaign';
				-- Add Action 
				insert into action_log
				(log_id,action_object,action_table_name,
				action_time,action_type,admin_user_email,admin_user_name,link_url)
