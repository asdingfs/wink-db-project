CREATE PROCEDURE [dbo].[Get_WINKTag_Ads_Tracker_Report]
(
  @url_name varchar(50),
...
select t.url_id,t.created_at,t.updated_at,t.id,t.url,t.customer_id,t.ip_address,img.campaign_name,
img.campaign_image_large as name, 
...
from winktag_ads_tracker as t
join winktag_campaign as img
on img.campaign_id = t.url_id
and (@campaign_name is null or (img.campaign_name like '%'+@campaign_name +'%'))
left join 
customer as c
on c.customer_id = t.customer_id
where 
--(@url_name is null or img.large_image_name like '%'+@url_name+'%')
--and 
...
from 
(select * from winktag_ads_tracker where CAST(created_at as date) BETWEEN 
CAST(@from_date as date) and CAST(@to_date as date)
and (@ip_address is null or winktag_ads_tracker.ip_address like @ip_address+'%')
