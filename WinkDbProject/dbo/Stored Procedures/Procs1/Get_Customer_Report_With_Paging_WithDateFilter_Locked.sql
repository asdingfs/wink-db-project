CREATE PROCEDURE [dbo].[Get_Customer_Report_With_Paging_WithDateFilter_Locked]      
 (@start_date datetime,      
  @end_date datetime,      
...
BEGIN     
	...
	IF (@start_date IS NOT NULL AND @end_date IS NOT NULL AND @start_date!='' AND @end_date !='' AND @status='disable')
	BEGIN 
		...
		BEGIN
			...
			BEGIN 
				...
				(                 				 
					...
					customer.email ,customer.[status],customer.WID, cusGroup.group_name as [group] from customer
					...
					join (select * from VW_Get_Disabled_Account (@start_date,@end_date)) as locked_account
					on customer.customer_id = locked_account.customer_id 
		...
		BEGIN
			...
			BEGIN
				;WITH                     
								customer_temp AS
								(
								 select a.customer_id,a.first_name,a.last_name ,a.email,a.[status],total_scans_,ip_address,ip_scanned,total_trips_,total_pts_issued_,total_wink_go_,
								  ROW_NUMBER() OVER(ORDER BY isnull(total_scans_,0) DESC) as intRow ,
									COUNT(a.customer_id) OVER() AS total_count,a.WID,a.[group] 
								 ...
								 (select 
									...
									on customer.customer_id = locked_account.customer_id 
								   Where  
								   customer.ip_address like '%'+ @ip_address+'%'
								   AND (@ip_scanned is null or customer.ip_scanned like '%'+ @ip_scanned+'%')
								AND (@wid is null or customer.WID like '%'+ @wid+'%')
								AND (@customer_name is null  or Lower(customer.first_name +' '+ customer.last_name) LIKE Lower( '%'+@customer_name +'%'))      
								AND (@customer_email is null  or Lower(customer.email) LIKE Lower('%'+LTRIM(RTRIM(@customer_email))+'%'))   
								...
								Group by customer_id
								   ...
								   Group By a.customer_id,a.first_name,a.last_name ,a.email,a.[status],total_scans_,ip_address,ip_scanned,total_trips_, total_pts_issued_,total_wink_go_,a.WID,a.[group]
			...
			BEGIN
				...
				customer_temp AS
				(
					select final.customer_id,final.first_name,final.last_name ,final.email,final.[status],final.total_scans_,final.ip_address,
					final.ip_scanned,final.total_trips_, final.total_pts_issued_,final.total_wink_go_,
					ROW_NUMBER() OVER(ORDER BY isnull(total_scans_,0) DESC) as intRow,
					total_count,final.WID,final.[group]
					FROM
					(
					...
					AND (@status is null  or Lower(customer.[status]) LIKE Lower(@status+'%'))
							...
							Group by customer_id
							   ...
							   Group By a.customer_id,a.first_name,a.last_name ,a.email,a.status,total_scans_,ip_address,ip_scanned,total_trips_,total_pts_issued_,total_wink_go_,a.WID,a.[group]
							   --Having ISNULL(total_trips_,0)+ ISNULL(total_scans_,0)>0
							   ) AS final
							   join 
							   customer on 
							   final.customer_id = final.customer_id
							   where intRow between @intStartRow and @intEndRow
							   Group By final.customer_id,final.first_name,final.last_name ,final.email,final.[status],total_scans_,final.ip_address,final.ip_scanned,final.total_trips_,final.total_pts_issued_,total_wink_go_,final.WID,final.[group],
							   total_count
								),  
