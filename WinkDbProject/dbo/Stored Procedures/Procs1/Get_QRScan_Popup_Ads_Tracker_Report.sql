CREATE PROCEDURE [dbo].[Get_QRScan_Popup_Ads_Tracker_Report]
(
   @url_name varchar(50),
...
from qrscan_popup_ads_tracker as t
...
where (@url_name is null or img.small_image_name like '%'+@url_name+'%')
...
from 
(select * from qrscan_popup_ads_tracker where CAST(created_at as date) BETWEEN 
CAST(@from_date as date) and CAST(@to_date as date)
and (@ip_address is null or qrscan_popup_ads_tracker.ip_address like '%'+@ip_address+'%')
