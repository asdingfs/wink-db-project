CREATE PROC [dbo].[Testing_Convert_Points_To_Winks_DependOn_CampaignAvaiable]
...
EXEC GET_CURRENT_SINGAPORT_DATETIME @CURRENT_DATETIME OUT
	...
	BEGIN 
		SELECT TOP 1 @CUSTOMER_ID = CUSTOMER_ID FROM CUSTOMER WHERE auth_token = @customer_tokenid 
	      ...
	      SELECT @REDEEMED_POINTS = @redeemed_winks*RATE_VALUE FROM RATE_CONVERSION WHERE RATE_CODE = 'points_per_wink'
			...
			BEGIN
				...
				(@CUSTOMER_ID,@merchant_id,@redeemed_winks,@REDEEMED_POINTS,@campaign_id,@CURRENT_DATETIME,@CURRENT_DATETIME)
				
				IF(@@ROWCOUNT>0)
				BEGIN
					UPDATE CUSTOMER_BALANCE SET USED_POINTS = USED_POINTS+@REDEEMED_POINTS, TOTAL_WINKS = TOTAL_WINKS+@redeemed_winks WHERE CUSTOMER_ID = @CUSTOMER_ID
...
/*ALTER PROC [dbo].[Convert_Points_To_Winks_DependOn_CampaignAvaiable]
@customer_tokenid VARCHAR(255),
@merchant_id int,
@campaign_id int
...
EXEC GET_CURRENT_SINGAPORT_DATETIME @CURRENT_DATETIME OUT
	...
	BEGIN 
		...
		AND CONVERT(CHAR(10),@CURRENT_DATETIME,111) <= CONVERT(CHAR(10),campaign.campaign_end_date,111))
