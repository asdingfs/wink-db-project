CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_NewLogic_beforeRemoveTrainInterval]          
...
BEGIN
 set @default_image_id =61		
 END


--NDP52 Campaign
	--1) Check 4K scan reach or not
	/*IF @qrcode = 'SMA_SMA_21_49653'
	BEGIN
		If CAST(@CURRENT_DATETIME as time) < '17:30:00'
        BEGIN
		   print('Time')
...
-- 2. Check Account Locked
    IF Exists (select 1 from customer where customer.auth_token = @customer_tokenid and status ='disable')
	 ...
	 SELECT '3' as response_code, 'Your account is locked. Please contact customer service.' as response_message , 3 as timer_interval_second
		RETURN 
	END-- END
...
-- End Safeguard----------------------------------------------------------------------------
			...
			BEGIN
				...
				BEGIN
					...
					print('Check one day per scan ')
				     IF(Cast(@CURRENT_SINGAPORE_TIME as date) <= Cast (@LAST_SCANNED_TIME as date) and @SCAN_INTERVAL!=0)
							BEGIN
							print('007 ')
	...
	/*-----------Start SMA----------------- 
	     IF @qrcode = 'SMA_SMA_21_49653'
	BEGIN
		set @SMALL_IMAGE_URL ='SMA_2017.jpg'
		set @SMALL_WEBSITE_URL =''
	END
	-----------Done SMA-----------*/
	   IF  ((select count(*) from customer_earned_points as p where p.qr_code =@qrcode and 
                ip_address =@ip_address and cast(p.created_at as date) = cast (@CURRENT_SINGAPORE_TIME as date))>1)
				BEGIN
				Declare @sameip_qr_interval int
				Declare @customer_id1 int
				Declare @customer_id2 int
				SELECT
					  top 1  @sameip_qr_interval=DATEDIFF(SECOND,[current].created_at,[next].created_at),@customer_id1 =[current].customer_id,
					   @customer_id2 = [next].customer_id
					   --,[next].created_at
					FROM
					   customer_earned_points       AS [current]
					 JOIN
					   customer_earned_points       AS [next]
   
					   ON [next].earned_points_id = (SELECT MIN(earned_points_id) FROM customer_earned_points WHERE earned_points_id > [current].earned_points_id
					   and cast(created_at as date ) = cast (@CURRENT_SINGAPORE_TIME as date)
					   ) 
					    and [current].ip_address =[next].ip_address
						and [next].qr_code  =[current].qr_code
						and [next].qr_code =@qrcode 
						and [next].ip_address =@ip_address

						 and cast([next].created_at as date ) = cast (@CURRENT_SINGAPORE_TIME as date)
						 and cast([current].created_at as date ) = cast (@CURRENT_SINGAPORE_TIME as date)
   
					   order by [next].created_at desc

				   if(@sameip_qr_interval <(60*5))
   
				   BEGIN
						 Print ('Locked same ip and qrcode')

							--Update customer set customer.status = 'disable',
							--customer.updated_at = @CURRENT_DATETIME where customer.customer_id = @customer_id1 or 
							--customer.customer_id = @customer_id2

							--IF (@@ROWCOUNT>0)
							--BEGIN
							--	Insert into System_Log (customer_id, action_status,created_at,reason)
							--	values (@customer_id1 ,'disable',@CURRENT_DATETIME,'SameIP_SameCode')

							--	Insert into System_Log (customer_id, action_status,created_at,reason)
							--	values (@customer_id2 ,'disable',@CURRENT_DATETIME,'SameIP_SameCode')
