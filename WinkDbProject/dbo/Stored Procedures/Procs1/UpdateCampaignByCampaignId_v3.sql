...
-- =============================================
CREATE PROCEDURE [dbo].[UpdateCampaignByCampaignId_v3]
	(
	 @campaign_id int,
	 @merchant_id int,
...
BEGIN
	...
	Begin
	 ...
	 @old_scan_limit = campaign.scan_limit,
	...
	IF EXISTS (SELECT campaign_id FROM campaign WHERE campaign.campaign_id = @campaign_id)
		BEGIN  
			...
			WHERE customer_earned_winks.campaign_id = @campaign_id GROUP BY campaign_id),0)
				...
				--3.CHECK UPDATE WINKS
				IF (@total_winks < @redeemed_total_winks and @campaign_id !=5)
						BEGIN 
	...
	BEGIN                                              
		BEGIN
				UPDATE campaign SET 
					...
					scan_limit = @old_scan_limit
					WHERE campaign_id =@campaign_id
					SELECT '0' as response_code, 'Fail to save the campaign log ' as response_message 
                     RETURN
						    END
					 ...
					 ELSE					
						...
						ELSE 
						BEGIN
							SELECT '0' as response_code, 'Fail to save the campaign' as response_message 
							RETURN
