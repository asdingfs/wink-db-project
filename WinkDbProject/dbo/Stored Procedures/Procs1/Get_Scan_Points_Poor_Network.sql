CREATE PROC [dbo].[Get_Scan_Points_Poor_Network]          
...
END
 SET @RETURN_NO='012' -- scan time is too frequent                           
	GOTO Err
END
...
BEGIN
  ...
  BEGIN --- Begin check time between 00:30 and 05:30
	...
	BEGIN                                              
		--SELECT '0' as response_code, 'Invalid Scan' as response_message , 1 as timer_interval_second
		--SELECT '0' as response_code, 'Invalid Scan' as response_message 
		SELECT '0' as response_code, 'Invalid Scan.May be due to poor network connectivity,account locked or daily maintenance after midnight' as response_message , 1 as timer_interval_second
		RETURN                           
	END 
	ELSE IF @RETURN_NO='002' 
	BEGIN  
		SELECT '0' as response_code, 'Scan interval per panel : 24 hours' as response_message , 1 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='003' 
	BEGIN  
		SELECT '0' as response_code, 'Invalid QR Code' as response_message , 1 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='004' 
	BEGIN  
		SELECT '0' as response_code, 'Insert Fail' as response_message , 1 as timer_interval_second
		RETURN 
	END
	...
	BEGIN  
		...
		, 1 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='012' 
	BEGIN  
		SELECT '0' as response_code,'Block' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
		, 1 as timer_interval_second
		RETURN 
	END
		...
		ELSE
		BEGIN
			SELECT '0' as response_code, 'Daily limit reached' as response_message , 1 as timer_interval_second
		RETURN 
...
ELSE
BEGIN
	SELECT '2' as response_code, 'Multiple logins not allowed' as response_message , 1 as timer_interval_second
...
--select * from customer where customer.email ='nnkk@gmail.com'

--update customer set customer.status = 'enable' where customer.email ='nnkk@gmail.com'
