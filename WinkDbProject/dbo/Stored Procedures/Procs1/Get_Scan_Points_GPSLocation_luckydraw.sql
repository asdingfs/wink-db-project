CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_luckydraw]          
...
DECLARE @WINNER_COUNT INT = 10;
Set @prize = 'You have won $38';
	...
	--select @prize = prize from hof_luckydraw where qr_code = @qrcode and customer_id =0
	 /*    print (@prize)
       ...
       if((select COUNT(*) from hof_luckydraw where customer_id = @customer_id and qr_code=@qrcode)=0 and @prize !='')
		...
		IF NOT EXISTS(SELECT * FROM hof_luckydraw WHERE qr_code = 'SingtelTVCNY_SingtelTVCNY_01_49190' AND CAST(created_at AS date) = CAST(@CURRENT_DATETIME as date))
		BEGIN

			If NOT EXISTS(SELECT * FROM hof_luckydraw WHERE qr_code = 'SingtelTVCNY_SingtelTVCNY_01_49190' AND CUSTOMER_ID = @CUSTOMER_ID)
			BEGIN
				Declare @count int

				IF((SELECT COUNT (*) from customer_earned_points 
				WHERE qr_code = 'SingtelTVCNY_SingtelTVCNY_01_49190'
