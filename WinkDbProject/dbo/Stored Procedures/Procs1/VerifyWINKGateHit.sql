CREATE PROCEDURE [dbo].[VerifyWINKGateHit]
(
...
BEGIN
 ...
 --   END
	...
	print(@campaignId)
	IF(@campaignId = @TLMCSilverCampaignId)
	BEGIN
		print('check silver daily limit')
		...
		IF (
   		...
   		BEGIN
			print('reach silver daily limit')
			SET @RETURN_NO='006';
			GOTO Err
		END
	END
	ELSE IF(@campaignId = @TLMCGoldenCampaignId)
	BEGIN
		print('check golden daily limit')
		--check for daily limit
		IF (
				(SELECT COUNT(*) 
					FROM wink_gate_points_earned  as e,
   		...
   		BEGIN
			print('reach golden daily limit')
			SET @RETURN_NO='006';
			GOTO Err
		END
	END
	...
	BEGIN
		...
		-- RTS KCD asset
		DECLARE @RTSKCDAssetId int = 390;
		-- RTS PLQ asset
		DECLARE @RTSPLQAssetId int = 380;
		if(@assetId = @RTSKCDAssetId OR @assetId = @RTSPLQAssetId)
		BEGIN
		IF(DATEDIFF(MINUTE,@lastLoggedTime, @CURRENT_DATETIME) < 2)
		BEGIN
			SET @RETURN_NO='004';
			GOTO Err
		END
