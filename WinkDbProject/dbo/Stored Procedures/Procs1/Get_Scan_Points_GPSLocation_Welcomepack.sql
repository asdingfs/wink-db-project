CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_Welcomepack]          
...
if(CAST(@CURRENT_DATETIME AS DATE) > CAST('2018-04-30' AS DATE))
BEGIN
...
where MONTH(created_at) = MONTH(@CURRENT_DATETIME)  
AND YEAR(created_at) = YEAR(@CURRENT_DATETIME)
AND qr_code = 'CitiSMRT_CitiSMRT_01_49658' ) >= 10)
BEGIN
...
END
	...
	IF Exists (SELECT 1 FROM customer_earned_points WHERE customer_earned_points.customer_id = @CUSTOMER_ID 
AND qr_code = 'CitiSMRT_CitiSMRT_01_49658')
BEGIN

SET @RETURN_NO='600' -- scan time is too frequent                           
GOTO Err
...
--AND CAST(can_id.created_at AS DATE) >= CAST('2018-03-27' AS DATE) AND CAST(can_id.created_at AS DATE) <= CAST('2018-03-28' AS DATE)
...
IF NOT Exists (SELECT 1 FROM customer_earned_points WHERE customer_earned_points.customer_id = @CUSTOMER_ID 
AND qr_code = 'CitiSMRT_CitiSMRT_01_49658')

BEGIN
IF Exists (SELECT 1 FROM welcome_citibank where welcome_citibank.corbrand_card IN( SELECT can_id.customer_canid from can_id WHERE can_id.customer_id = @CUSTOMER_ID 
AND CAST(can_id.created_at AS DATE) >= CAST('2018-03-27' AS DATE) AND CAST(can_id.created_at AS DATE) <= CAST('2018-04-30' AS DATE)
AND SUBSTRING(customer_canid,1,6) = '100930') )
BEGIN
	...
	SET @RETURN_NO='309' -- Already get points using these can ids                      
	GOTO Err
...
AND SUBSTRING(customer_canid,1,6) = '100930')
    ...
    INSERT INTO welcome_citibank (customer_id,campaign_booking_id,campaign_id,points,last_scanned_time,qr_code,created_at,ip_address,GPS_location, corbrand_card, registered_date_for_corbrand_card) VALUES
...
SET @RETURN_NO='600' -- Already Scan                      
	GOTO Err
...
BEGIN
	...
	SET @RETURN_NO='302' -- Over Limited Per Month                       
	GOTO Err
...
IF Exists (SELECT 1 FROM welcome_citibank where welcome_citibank.corbrand_card IN( SELECT top 1 can_id.customer_canid from can_id WHERE can_id.customer_id = @CUSTOMER_ID 
AND CAST(can_id.created_at AS DATE) >= CAST('2018-03-27' AS DATE) AND CAST(can_id.created_at AS DATE) <= CAST('2018-04-30' AS DATE)
AND SUBSTRING(customer_canid,1,6) = '100930') )
...
END
	...
	BEGIN  
		SELECT '1' as response_code,'Congrats! You’ve earned 400 WINK+ points which will be credited to your WINK+ account. You can view your points under Account > My Rewards.' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
	...
	BEGIN  
		SELECT '0' as response_code, 'Please link your Citibank SMRT Card to your WINK+ account.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	ELSE IF @RETURN_NO='302' -- Over Limited Per Month 
	BEGIN  
		SELECT '0' as response_code, 'Promotion is fully redeemed this month. Please try again next month!' as response_message , 3 as timer_interval_second
		RETURN 
	END
	...
	--BEGIN  
		...
		--RETURN 
	--END
	ELSE IF @RETURN_NO='301' -- Over Limited Per Month 
	BEGIN  
		SELECT '0' as response_code, 'Sorry, this promotion has now ended.' as response_message , 3 as timer_interval_second
		RETURN 
	END
