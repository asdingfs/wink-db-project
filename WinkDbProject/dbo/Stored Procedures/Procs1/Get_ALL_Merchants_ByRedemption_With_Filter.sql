CREATE  PROCEDURE [dbo].[Get_ALL_Merchants_ByRedemption_With_Filter]
...
if (@start_date is NULL or @end_date is NULL or @start_date = '' or @end_date = '')
  Select * from 
...
ELSE
	 ...
	 AND ( ((cast(eVoucher_transaction.created_at as date) >= cast(@start_date as date)) and (cast(eVoucher_transaction.created_at as date) <= cast(@end_date as date))))
