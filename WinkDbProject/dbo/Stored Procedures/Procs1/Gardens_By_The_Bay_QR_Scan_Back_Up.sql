CREATE PROC [dbo].[Gardens_By_The_Bay_QR_Scan_Back_Up]          
...
--- check last day of straw winner						 
											IF(Cast(@CURRENT_DATETIME as date) != @qrcode_end_date)
												BEGIN
													...
													BEGIN
	--- check if staw already full , check 10 point
											...
											ELSE 
											    BEGIN
													...
													BEGIN
--- check last day if staw already full , check 10 point
													--SET @isWinner = '0'
														IF(
															(SELECT COUNT(*) FROM qr_campaign
															WHERE campaign_id = @winktagId
...
--- check 10 p already full
											--	SET @isWinner = '0'
												...
												ELSE
													...
													END
											END
--- check last day of 10 points winner
											IF(
											(SELECT COUNT(*) FROM qr_campaign
											WHERE campaign_id = @winktagId
											AND winning_status = '1'
											...
											BEGIN
--- check last day 10 already full ,give straw
												--SET @isWinner = '0'
...
----	end < 1000			
					ELSE
						BEGIN
							SET @RETURN_NO='010'   -- IFully redeemed                     
							GOTO Err
						END
