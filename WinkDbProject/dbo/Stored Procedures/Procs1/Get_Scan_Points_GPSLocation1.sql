Create PROC [dbo].[Get_Scan_Points_GPSLocation1]          
...
If CAST(@CURRENT_DATETIME as time) > '00:00:00' 
   ...
   print('Time')
	...
	SELECT '0' as response_code, 'Invalid Scan1' as response_message , 3 as timer_interval_second
		RETURN 
	END-- END check time between 00:30 and 05:30
-- 1. Check Multiple Login
...
AND  (select SUBSTRING ( @qrcode, 1 , 9) ) = SUBSTRING ( qr_code, 1, 9 )
  and CAST (created_at as DATE) = CAST (@CURRENT_DATETIME as Date)) >= 24
  )
		...
		BEGIN
			Insert into System_Log (customer_id, action_status,created_at,reason)
			Select customer.customer_id,
			'disable',@CURRENT_DATETIME,'Same Train Scan Count <= 24'
			 from customer where customer.auth_token = @customer_tokenid
		END
		 SET @RETURN_NO='001' -- scan time is too frequent                           
			GOTO Err
...
-- End Safeguard----------------------------------------------------------------------------
	...
	BEGIN  -- 4.. Check customer is enable
	    ...
	    SELECT TOP 1 @CUSTOMER_ID = CUSTOMER_ID FROM CUSTOMER WHERE auth_token = @customer_tokenid 
	       ...
	       BEGIN
			...
			/*INSERT INTO CUSTOMER_EARNED_POINTS_Log (customer_id,campaign_booking_id,campaign_id,points,last_scanned_time,qr_code,created_at) VALUES
					...
					*/
			INSERT INTO CUSTOMER_EARNED_POINTS_Log (customer_id,campaign_booking_id,campaign_id,points,last_scanned_time,qr_code,created_at) VALUES
			(@CUSTOMER_ID,0,0,0,@CURRENT_SINGAPORE_TIME,@qrcode,@CURRENT_SINGAPORE_TIME)		
			...
			-- Check 24 Hours QR scan interval	
	       ...
	       END -- End Station Interval Check
	           
		-- GET Required Field  @CAMPAIGN_ID  @BOOKING_ID @SCAN_INTERVAL @SCAN_VALUE
		-- @SMALL_IMAGE_URL  @SMALL_WEBSITE_URL  @CUSTOMER_ID
		-- IF asset is booked
		...
		BEGIN
		  ...
		  --a1.. Asset is booked 
			...
			-- a2-- Get small image and url 
			
			SELECT @SMALL_IMAGE_URL = booking_image_name ,@SMALL_WEBSITE_URL = booking_image_url FROM booking_image WHERE booking_id = @BOOKING_ID
			and booking_image_status = '1'
			print(@SMALL_WEBSITE_URL)
			
			-- a3 Get Merchant ID
			SELECT @MERCHANT_ID = MERCHANT_ID FROM CAMPAIGN WHERE CAMPAIGN_ID = @CAMPAIGN_ID
			
			-- a4 It is valid qr code
			
			Set @Valid = 1
		
		END
		ELSE -- IF Asset is not booked check global
		BEGIN
		...
		END
			...
			IF @Valid= 0 -- Qr code is  not vlid to scan 
	...
	BEGIN                                              
		SELECT '0' as response_code, 'Invalid Scan2' as response_message , 3 as timer_interval_second
	...
	BEGIN  
		SELECT '0' as response_code, 'Invalid Scan3' as response_message , 3 as timer_interval_second
		RETURN 
	END
