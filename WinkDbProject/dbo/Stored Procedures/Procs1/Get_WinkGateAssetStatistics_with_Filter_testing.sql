CREATE PROCEDURE [dbo].[Get_WinkGateAssetStatistics_with_Filter_testing]
(
	@from_date datetime,
	@to_date datetime,
	@gate_id VARCHAR(100),
	@gate_description VARCHAR(100),
...
BEGIN
   ...
   IF(@gate_description is null or @gate_description ='')

	SET @gate_description = NULL;
	ELSE
	SET @gate_description = UPPER(@gate_description)
 ...
 ;with tbl as (
	SELECT * FROM(
		...
		AND (@gate_description is null or UPPER(a.[description]) like '%'+@gate_description+'%')
	...
	,tbl2 as (
	 ...
	 ,tbl3 as (
		...
		wink_gate_campaign as wc,
		campaign as c
		 where a.wink_gate_booking_id = b.id  
		 AND b.wink_gate_campaign_id = wc.id
		AND wc.campaign_id = c.campaign_id
		AND (@campaign_name is null or UPPER(c.[campaign_name]) like '%'+@campaign_name+'%')
		AND  (@from_date is null or @to_date is null or (cast(a.created_at as date) >=  cast (@from_date as date)
		...
		--and b.wink_gate_asset_id = 16 and a.campaign_id=1670
	 ...
	 (b.total_hits*(@reach_multipler+1) ) as total_footfall,
	  b.impressions, 
	   ...
	   left join tbl3 as c on (a.assetId = c.assetId and a.campaign_id = c.campaign_id)
		...
		--where ROW_NUMBER() OVER(order by b.total_hits desc) between @intStartRow and @intEndRow
	   --order by b.total_hits desc
	   )
	   

	 --sumHits = sumHits + item.Total_hits;
       --sumReach = sumReach + item.Total_reach;
       --sumFootfall = sumFootfall + item.Total_footfall;
       -- sumRespondents = sumRespondents + item.Respondents;
        -- sumImpressions = sumImpressions + item.Impressions;
        --   sumEngagement = sumEngagement + item.Engagements;
        --sumTotalPoints = sumTotalPoints + item.Total_points;
	  ...
	  (
	   ...
	   --join  selectMAX(intRow)/@intPageSize) as total_pages
END