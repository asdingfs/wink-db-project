CREATE PROCEDURE [dbo].[Confirm_eVoucher_Transaction_By_WINKProduct_SendEmail]
	(
	...
	and CAST(customer_earned_evouchers.expired_date as Date) > CAST(@current_date as Date)
			...
			BEGIN
				...
				IF NOT EXISTs (select 1  from wink_products as p where
				 p.branch_id=@branch_code and price = @EVOUCHER_VALUE )
				 BEGIN
				 	set @return_code = '003'
					Goto Err
				 ...
				 select @product_id = isnull(id,0) from wink_products as p where
				 p.branch_id=@branch_code and price = @EVOUCHER_VALUE 
				 and product_status =1
				 --and qty>redeemed_qty

				
				IF (@product_id is not null and @product_id !=0  and @EVOUCHER_VALUE is not null and @EVOUCHER_VALUE !=0)
					BEGIN
					...
					if ( (select COUNT(*) from wink_products_redemption where product_id = @product_id and thirdparty_eVoucher_id !=0) =
                        ...
                        OR ((select COUNT(*) from wink_products where qty = redeemed_qty and id = @product_id)>= 1)                                             
                        )
                        BEGIN
								...
								BEGIN
									...
									--CHECK eVOUCHER IS VALID
										...
										BEGIN
												...
												(select top 1 isnull(id,0) from wink_products_thirdparty_evoucher where product_id = @product_id
												 and id not in (select thirdparty_eVoucher_id from wink_products_redemption where thirdparty_eVoucher_id !=0 ))
																								
												)
												
											    select @thirparty_evoucher_id = thirdparty_eVoucher_id from wink_products_redemption where eVoucher_id = @evoucher_id
											    
											    --- Update thirdparty voucher used status
											    
											    update wink_products_thirdparty_evoucher set used_status = 1,updated_at= @current_date where id = @thirparty_evoucher_id
											    
											    -- Select Third Party evoucher code
												select @eVoucher_code = eVoucher_code from wink_products_thirdparty_evoucher where 
												id = @thirparty_evoucher_id
											...
											@current_date
												...
												SELECT '1' AS	success , @success_message as response_message,@eVoucher_code as thirdparty_code,
						                         @TRANS_REF_NO AS transaction_id,@CUSTOMER_EMAIL as email,@MERCHANT_ID as merchant_id
