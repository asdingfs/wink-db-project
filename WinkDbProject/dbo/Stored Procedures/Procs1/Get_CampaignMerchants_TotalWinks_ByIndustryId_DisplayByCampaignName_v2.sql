CREATE PROCEDURE [dbo].[Get_CampaignMerchants_TotalWinks_ByIndustryId_DisplayByCampaignName_v2]
(
@industryId int
)
AS
...
-- Update on 09/11/2016  - Display Image from Campaign
...
IF OBJECT_ID('tempdb..#CAMPAIGN_2') IS NOT NULL DROP TABLE #CAMPAIGN_2
	...
	LEFT JOIN (select * from campaign_small_image where campaign_small_image.small_image_status =1) as campaign_small_image
	ON 
	campaign_small_image.campaign_id=#CAMPAIGN_2.campaign_id
	and campaign_small_image.small_image_status =1
	LEFT JOIN campaign_large_image
	ON 
	campaign_large_image.campaign_id=#CAMPAIGN_2.campaign_id
	...
	-- remove end date  Update on 20062016
	...
	-- Remove 0 except WINK + 20062016
	OR #CAMPAIGN_2.campaign_id = 5
	
	
	GROUP BY #CAMPAIGN_2.merchant_id,
	campaign_small_image.small_image_name,campaign_large_image.large_image_name,campaign_large_image.large_image_url,
	campaign_large_image.id,	
	merchant.first_name,merchant.last_name,campaign.campaign_name,#CAMPAIGN_2.campaign_id
