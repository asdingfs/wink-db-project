CREATE PROC [dbo].[Game_Scan_QRcode]          
 ...
 BEGIN --- Begin check time between 00:30 and 05:30
	...
	BEGIN  
		...
		BEGIN
			...
			-- Check Valid Check Point / QR Code  *****************************
				...
				IF EXISTS (Select * from game_player_checkpoints_summary where team_id = @teamId)
					...
					-- 3. Check Valid Next QR Scan 
						IF (@qrcode != (select game_checkpoint_mapping.to_checkpoint_qr
						 from game_checkpoint_mapping
						 where game_checkpoint_mapping.from_checkpoint_qr =@last_team_qrscan))
							BEGIN
							--SET @Valid = 0
							SET @RETURN_NO='006' -- scan time is too frequent                           
							GOTO Err
							END
			...
			/*Select @SMALL_WEBSITE_URL = game_checkpoint_detail.game_hint_url from game_checkpoint_detail
			...
			BEGIN
				...
				print(@DATE_DIFF);
				print (@SCAN_INTERVAL);
				--SET @SCAN_INTERVAL =0 -- Set Scan Interval to '0'
	...
	BEGIN  
		SELECT '0' as response_code, 'Invalid Scan Check Point Locaton' as response_message 
		RETURN 
	END
