CREATE PROCEDURE [dbo].[Get_eVoucherDetail_By_VerificationCode_ThirdPartyKey]
(
...
/*------------------------Merchant Authentication---------------------*/
	...
	SET @merchant_id = (Select thirdparty_authentication.merchant_id from thirdparty_authentication where thirdparty_authentication.secret_key =@secret_key)
		   ...
		   SET @evoucher_id =( SELECT 
			...
			IF @evoucher_id IS NOT NULL AND @evoucher_id !='' AND @evoucher_id!=0
				BEGIN
					...
					e.eVoucher_code,1 as success,'Valid merchant key and verification code' as response_message
			...
			ELSE 
				BEGIN
				 SELECT 0 AS success ,'Invalid merchant key or varification code' As response_message
				END
