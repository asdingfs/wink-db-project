CREATE PROC [dbo].[Get_Scan_Points_GPSLocation_AgencyGame]          
...
END
else if(@qrcode like '%iView%')
BEGIN
 set @default_image_id =65	
END
 
 -- Check for Agency Code

 If CAST(@CURRENT_DATETIME as time) < '10:00:00' 
   AND CAST(@CURRENT_DATETIME as time) >= '18:00:00'
   BEGIN
    
	if (@qrcode like '%Agency_Staff%')
	BEGIN
   
	SELECT '0' as response_code, 'Only allow to scan during the limitation time' as response_message , 3 as timer_interval_second
		RETURN 
   ...
   END
    ...
    if (@qrcode like '%Agency_Staff%')
	BEGIN
	 IF EXISTS (select 1 from customer_earned_points where qr_code = @qrcode and  customer_id = (select customer_id from 
	 customer where customer.auth_token = @customer_tokenid ))
	 BEGIN
	  Print (2)
	 SELECT '0' as response_code, 'Limit to one scan per campaign' as response_message , 3 as timer_interval_second
	 RETURN 
	 END
	 PRINT (1)
     IF  EXISTS (select 1 from customer where customer.auth_token = @customer_tokenid 
	 and customer.customer_id not in (select customer_id from agency_game_customers))
	 BEGIN
	 Print (2)
	 SELECT '0' as response_code, 'By invitation only' as response_message , 3 as timer_interval_second
	 RETURN 
	END
...
IF ( (select group_id from agency_game_customers where customer_id =@customer_id ) != 
      (select agency_id from agency_qr_code where qr_code = @qrcode))

	  BEGIN
	     if (@qrcode not like '%Agency_Staff%')
		 BEGIN
	      SELECT '0' as response_code, 'By invitation only' as response_message , 3 as timer_interval_second
          RETURN 
		END      
...
-- End Safeguard----------------------------------------------------------------------------
	...
	ELSE IF @RETURN_NO='000' 
     BEGIN  
		...
		IF EXISTs (select 1 from agency_game_winner_timing as t 
		where t.date_status = 1 and cast(@CURRENT_DATETIME as datetime)  >= cast(t.time_from as datetime) 
		and cast(@CURRENT_DATETIME as datetime) <= cast(t.time_to as datetime)
		and t.qr_code = @qrcode
		)
		BEGIN
		
		   IF NOT Exists (select 1 from hof_luckydraw where customer_id = @CUSTOMER_ID and event_name = 'AgencyGame072017')
	    BEGIN
		   IF Exists ( select  group_id from agency_game_customers where customer_id = @CUSTOMER_ID 
			and group_id not in (select group_id from agency_game_customers as a ,hof_luckydraw as h
			where h.customer_id = a.customer_id
			and cast(h.created_at as date) = cast(@CURRENT_SINGAPORE_TIME as date)))

			BEGIN
		
				--select @prize = prize  from [lucky_draw_price] where cast(created_at as date) = cast(@CURRENT_SINGAPORE_TIME as date)
				select Top 1 @prize = prize from [lucky_draw_price] where event_name = 'AgencyGame072017'
				
				IF (@prize is not null and @prize !='')
				BEGIN
				SET @winner = 'Congratulations! You have won ' + @prize +'! An email will be send with the redemption details!'
				...
				INSERT INTO [dbo].[hof_luckydraw]
						...
						VALUES
							(@qrcode
							,@CUSTOMER_ID
							,@CURRENT_SINGAPORE_TIME--@CURRENT_SINGAPORE_TIME
							,@CURRENT_SINGAPORE_TIME--@CURRENT_SINGAPORE_TIME
							,@prize
							,'AgencyGame072017'
							)
