...
CREATE PROC [dbo].[Get_NETS_Contactless_Report]
(
...
set @CAMPAIGN_ID =26
	   ...
	   Select * from (
			...
			SELECT w.nets_card as nets_card , w.registered_date_for_nets as nets_card_created, 
			'' as answer,
			...
			FROM [NETS_Contactless_Cashcard] as w 
		    ...
		    ( @gender is null OR c.gender =@gender ) 
			  ...
			  AND 
		    ( @netscanid is null OR w.nets_card =@netscanid ) 
			-------UNION WITH WINK TAG LOG---------------------------

			 UNION

			SELECT '' as nets_card,NULL as nets_card_created,
			'' as answer,
			A.created_at as participated_date,
		   ...
		   'No' as correct_answer_status
			 ...
			 AND A.customer_id not in (select distinct customer_id from [NETS_Contactless_Cashcard]
