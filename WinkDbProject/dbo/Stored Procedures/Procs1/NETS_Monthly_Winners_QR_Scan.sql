CREATE PROCEDURE [dbo].[NETS_Monthly_Winners_QR_Scan]          
...
-- they can only scan once
if ((SELECT count(*) from customer_earned_points where customer_id = @CUSTOMER_ID AND qr_code = 'NETSLuckyDraw_NETSLuckyDraw_NETSLuckyDraw_33944' ) > 0)
BEGIN
...
--BEGIN
Declare @listOfWinners TABLE(id int);
--(3963) hanny
...
-- NETs Monthly Lucky Draw Winner
if ((SELECT count(*) from customer_earned_points where customer_id = @CUSTOMER_ID AND qr_code = 'NETSLuckyDraw_NETSLuckyDraw_NETSLuckyDraw_33944' ) = 0)
BEGIN
...
--Err
	...
	BEGIN  
		SELECT '1' as response_code,'Congrats! You have received 1000 WINK+ points which will be credited to your WINK+ account. You can view your points under Account > My Rewards.' as response_message, @SMALL_IMAGE_URL AS small_banner_url, @SCAN_VALUE as scan_value,@SMALL_WEBSITE_URL AS small_website_url
	...
	BEGIN  
		--SET @SMALL_IMAGE_URL = ''
		SELECT '0' as response_code, 'You have already claimed your prize for the Use & Win with NETS Contactless CashCard Lucky Draw.' as response_message , 3 as timer_interval_second
		RETURN 
	END
	...
	BEGIN  
		...
		RETURN 
	END
	ELSE IF @RETURN_NO='310' -- Invalid User for NETS Monly Lucky Draw Prize
	BEGIN  
		SELECT '0' as response_code, 'Sorry! You are not eligible to receive this prize.' as response_message , 3 as timer_interval_second
		RETURN 
	END
