CREATE PROC [dbo].[WINK_Game_Simplified_Report]
(
...
BEGIN
	...
	BEGIN
		...
		(
			...
			T.points, T.winner, T.Q1,T.GPS_location, T.ip_address, T.created_at
			
			FROM
			(
				-----table1
				SELECT * from
				(SELECT l.customer_id,0 as points, '0' as winner, l.gps_location as GPS_location,l.ip_address,l.created_at, '' as Q1
				...
				-----table2
				SELECT * from(
				select r.customer_id,r.point as points, r.winner as winner, r.gps_location as GPS_location, r.ip_address, r.created_at, o.option_type as Q1
	...
	BEGIN
		...
		(
			SELECT ROW_NUMBER() OVER (Order by T.created_at ASC)AS no,c.first_name +' '+c.last_name as customer_name,c.gender as gender, (select floor(datediff(day,c.date_of_birth, DATEADD(HOUR,8,GETDATE())) / 365.25)) as age, c.email,c.WID as wid, c.status as status, c.customer_id as customer_id,
			T.asset_name, T.points, T.winner, T.Q1,T.GPS_location, T.ip_address, T.created_at, T.redemption_code, T.redemption_status,T.redeemed_on, T.redemption_location
			...
			FROM
					(
						...
						-----table1
						SELECT * from
						(SELECT l.customer_id,0 as points, '0' as winner, l.gps_location as GPS_location,l.ip_address,l.created_at, '' as Q1, s.asset_name,
						...
						-----table2
						SELECT * from(
						select r.customer_id,r.point as points, r.winner as winner, r.gps_location as GPS_location, r.ip_address, r.created_at, o.option_answer as Q1, s.asset_name,
