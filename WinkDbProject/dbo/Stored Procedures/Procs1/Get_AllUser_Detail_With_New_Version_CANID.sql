...
CREATE PROCEDURE [dbo].[Get_AllUser_Detail_With_New_Version_CANID]
	 (@email varchar(150),
	  @name varchar (200),
	  @status varchar(20),
...
BEGIN
	...
	If (@subscribe_status is null OR @subscribe_status ='')
	BEGIN
		set @subscribe_status = NULL
	END
	...
	IF (@skin is null OR @skin = '' OR @skin='All')
		set @skin = NULL

	IF (@avartar is null OR @avartar = '' OR @avartar = 'All')
		set @avartar = NULL
	 ...
	 print('Filter Locked')
		...
		BEGIN
			select * from (
				...
				unique_can_id.card_tag as card_tag,
				ISNULL(cusCard.cardType,'NA') as cardType
				from customer
				join wink_team
				...
				left join (
					select * from can_id
					where id in (
						select max(id) from can_id 
						where (@canid is null or (customer_canid like '%' + @canid + '%'))
						group by customer_id
					)
				) as unique_can_id
				on (customer.customer_id = unique_can_id.customer_id)
				left join customer_card_types as cusCard
				ON customer.customer_id = cusCard.customerId
				--------- Join Lock Account
				Join 
				(
					...
					(
						...
						OR (
							customer_id in 
							(
								select customer_id from System_Log where action_status ='disable'
				...
				AND (@email is null or Lower(customer.email) LIKE Lower('%'+LTRIM(RTRIM(@email))+'%') )			
				and (@status is null or customer.[status] Like @status+'%')
				and  (@subscribe_status is null or  subscribe_status Like @subscribe_status+'%')
				and (@skin is null or customer.skin_name like @skin+'%')
				and (@team is null or wink_team.team_name like @team+'%')
				and (@avartar is null or avatar.avatar_name like @avartar +'%')
			    ...
			    AND (@cardType IS NULL
						OR (cusCard.cardType LIKE @cardType AND @cardType NOT LIKE 'NA')
						OR (@cardType like 'NA' AND cusCard.cardType IS NULL)
					)         
				) as temp  
		...
		BEGIN
			select * from (
				...
				join (
						...
						from customer as w 
						where EXISTS  
						(select customer_id  FROM can_id where customer_canid like '%'+@canid +'%')
				) as can
	...
	BEGIN
		...
		select * from (
			...
			customer.WID AS W_id,
			customer.date_of_birth,customer.email,customer.first_name,customer.last_name,
			(select floor(datediff(day,customer.date_of_birth, DATEADD(HOUR,8,GETDATE())) / 365.25)) as age,
			customer.[password],customer.gender,
			...
			and (
				@canid is null OR 
				customer.customer_id in (select customer_id  FROM can_id where can_id.customer_canid like @canid +'%')
			)
			AND (@cardType IS NULL
				OR (cusCard.cardType LIKE @cardType AND @cardType NOT LIKE 'NA')
				OR (@cardType like 'NA' AND cusCard.cardType IS NULL)
