CREATE PROC [dbo].[Auth_Covert_Points_To_WINKs]
...
BEGIN
	...
	DECLARE @Today_redeemed_winks int
	DECLARE @total_today_redeemed_winks int 
	SET @total_today_redeemed_winks =0;
	SET @Today_redeemed_winks=0;
   ...
   END
	...
	IF  EXISTS (Select 1 From customer where auth_token = @customer_tokenid and status ='disable')
    BEGIN
	...
	IF Exists (
		...
		(
			ISNULL(customer.phone_no,'') =''
			OR ISNULL(customer.gender,'')='' 
			OR ISNULL(customer.date_of_birth,'')=''
		)
	...
	IF EXISTS (
		select 1 from customer_earned_winks 
		where CAST(created_at as date) = CAST (@CURRENT_DATETIME as date)
		and customer_id = @CUSTOMER_ID 
	)
	BEGIN
		SELECT @Today_redeemed_winks =sum(ISNULL(total_winks,0))
		FROM customer_earned_winks as w 
		WHERE CAST(created_at as date) = CAST (@CURRENT_DATETIME as date)
		AND customer_id = @CUSTOMER_ID;

		SET @total_today_redeemed_winks =@Today_redeemed_winks;
	END

	SET @Today_redeemed_winks = @WINKS + ISNULL(@Today_redeemed_winks,0);		

	IF (@Today_redeemed_winks>20)
	BEGIN
		SELECT '0' as response_code, ' Daily limit of 20 WINKs redemption.' as response_message
		RETURN
	END
	...
	BEGIN 
		...
		ELSE 
		BEGIN
			Declare @response_message varchar(200);

			SET @response_message = '(Daily limit of 20 WINKs redemption) Do you want to proceed?';

			SELECT '1' as response_code,  @response_message as response_message 
			RETURN
		END
	END
END