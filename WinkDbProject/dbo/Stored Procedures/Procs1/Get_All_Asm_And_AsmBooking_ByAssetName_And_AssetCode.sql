CREATE PROCEDURE [dbo].[Get_All_Asm_And_AsmBooking_ByAssetName_And_AssetCode]
...
BEGIN
	...
	where Lower(asset_type_management.asset_code)LIKE Lower('%'+@asset_code+'%')AND
	Lower(asset_type_management.asset_name)LIKE Lower('%'+@asset_name+'%')
