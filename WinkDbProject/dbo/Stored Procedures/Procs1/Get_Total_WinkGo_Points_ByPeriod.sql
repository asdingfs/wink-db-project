CREATE PROCEDURE [dbo].[Get_Total_WinkGo_Points_ByPeriod]
...
BEGIN
	...
	SELECT COUNT(*) AS total_scans, RIGHT('0'+ CONVERT(VARCHAR(2), Month(created_at)), 2) + '-' + CAST(YEAR(created_at) AS VARCHAR(4)) AS period FROM nonstop_net_canid_earned_points 

	WHERE  CAST(created_at As Date) >= CAST (@from_date AS Date) and CAST(created_at As Date) <= CAST (@to_date AS Date)
	and points_credit_status = 1
