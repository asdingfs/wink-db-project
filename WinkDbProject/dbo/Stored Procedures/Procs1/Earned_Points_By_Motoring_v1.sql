CREATE procedure [dbo].[Earned_Points_By_Motoring_v1]
(
...
Declare @global_campaign int
	...
	IF EXISTS (Select 1 from ASSET_WINKGO  where cast( (select dateadd(HOUR,interval,to_date)) as date) >=  cast(@current_date as date) and cast(from_date as date) <=  cast(@current_date as date) 
