...
CREATE PROC [dbo].[Get_Scan_Points_Zin_Win]          
...
BEGIN 
	...
	BEGIN  
		...
		--IF EXISTS (SELECT * FROM game_team_players_details WHERE customer_id = @CUSTOMER_ID AND team_id = @teamId AND event_date_id = @event_id)
		
		--CHECK customer is already registered or not
		IF EXISTS (SELECT * FROM game_team_players_details WHERE customer_id = @CUSTOMER_ID)
		BEGIN 
			...
			IF NOT EXISTS (SELECT * FROM game_team_players_details WHERE customer_id = @CUSTOMER_ID AND team_id = @teamId AND event_date_id = @event_id)
			BEGIN
				SET @RETURN_NO='008' -- Invalid team player in team                     
				GOTO Err
			END
		...
		ELSE
		BEGIN
			SET @RETURN_NO='007' -- Customer is not registerd for game                      
			GOTO Err
		END
		--*******************START GAME***********************************************************
			...
			---Select (select game_checkpoint_mapping.to_checkpoint_qr from game_checkpoint_mapping
			---where game_checkpoint_mapping.from_checkpoint_qr = @qrcode) as to_qr_code_from_game_checkpoint_mapping

			SELECT @SMALL_IMAGE_URL = game_checkpoint_detail.small_image from game_checkpoint_detail
			...
			--Select @SMALL_WEBSITE_URL as q2
			-- Insert Into Log Table 
	...
	BEGIN  
		...
		RETURN 
	END
	ELSE IF @RETURN_NO='007' 
	BEGIN  
		SELECT '0' as response_code, 'Invalid team player' as response_message 
		RETURN 
	END
	ELSE IF @RETURN_NO='008' 
	BEGIN  
		SELECT '0' as response_code, 'Invalid team player in team' as response_message 
		RETURN 
	END
	...
	BEGIN  
		...
		BEGIN
			...
			IF NOT EXISTS (SELECT * FROM game_player_checkpoints_detail WHERE qr_code = @qrcode AND customer_id = @CUSTOMER_ID and team_id = @teamId and event_id = @event_id)
			BEGIN
				Insert into game_player_checkpoints_detail (customer_id,qr_code,created_at,event_id,event_date,team_id,campaign_booking_id,points,last_scanned_time,campaign_id)
				values (@CUSTOMER_ID,@qrcode,@CURRENT_DATETIME,@event_id,@CURRENT_DATETIME,@teamId,@BOOKING_ID,(select scan_value from asset_management_booking where qr_code_value= @qrcode),@CURRENT_DATETIME,(select campaign_id from asset_management_booking where qr_code_value= @qrcode))
				...
				and game_player_checkpoints_detail.qr_code = @qrcode and event_id = @event_id) =
				(Select COUNT(*) from game_team_players_details where game_team_players_details.team_id = @teamId AND event_date_id = @event_id))
				BEGIN
				...
				<= (SELECT COUNT(*) FROM game_checkpoint_detail WHERE EVENT_ID = @event_id))
				BEGIN
					IF((SELECT COUNT(*) FROM game_player_checkpoints_summary WHERE TEAM_ID = @teamId AND EVENT_ID = @event_id)
					...
					ELSE
					BEGIN
						IF ((SELECT COUNT(*) FROM game_player_checkpoints_summary WHERE team_id = @teamId AND event_id = @event_id) = (SELECT COUNT(*) FROM game_checkpoint_detail WHERE EVENT_ID = @event_id))
						BEGIN
							INSERT INTO game_player_milestone_complete (team_id,milestone_number,created_at,event_id)
							values(@teamId,(SELECT COUNT(*) FROM game_player_milestone_complete WHERE TEAM_ID = @teamId AND EVENT_ID = @event_id)+1,GETDATE(),@event_id)
						END
