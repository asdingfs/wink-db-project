CREATE PROCEDURE [dbo].[Get_WINKTag_ByQRCode_testing]
(
...
-- End Safeguard----------------------------------------------------------------------------
	...
	BEGIN  -- 4.. Check customer is enable
		...
		IF EXISTS(SELECT 1 FROM customer_earned_points_by_winktag WHERE CUSTOMER_ID = @CUSTOMER_ID AND QR_CODE = @qrcode)  
			BEGIN
				...
				SET @RETURN_NO='000' -- scan time is too frequent                           
				GOTO Err
	...
	IF @RETURN_NO ='000'
	BEGIN
		select c.campaign_name,c.campaign_id,b.asset_type_management_id,b.booking_id,b.scan_value,w.video_name,w.survey_url,w.winktag_type,w.wink_content,
