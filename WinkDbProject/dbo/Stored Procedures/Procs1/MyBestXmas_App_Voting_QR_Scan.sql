CREATE PROC [dbo].[MyBestXmas_App_Voting_QR_Scan]
...
BEGIN 
	...
	ELSE
	BEGIN
		print('invalid QR code category')
		/******************************************************/
		SET @RETURN_NO='003'   -- INVALID QR CODE                        
		GOTO Err
	END    
		...
		BEGIN TRY
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							SET @RETURN_NO='007' -- WINK+ Play banner has already been enabled  
						END
						ELSE
						BEGIN
							SET @RETURN_NO='008' -- User has already voted for the day       
						END
					END
					ELSE 
					BEGIN
			...
			BEGIN
				...
				BEGIN
					SET @RETURN_NO='000'
				END
				print('first time insert record')
			END
	...
	BEGIN  
		...
		RETURN 
	END
	ELSE IF  (@RETURN_NO='007' or @RETURN_NO='008' or @RETURN_NO='001') 
	BEGIN  
		set @default_image_id = 478;

		IF(@RETURN_NO='007')
		BEGIN
			-- WINK+ Play banner has already been enabled
			set @successMsg = 'Head to WINK+ Play to finish voting for your favourite video!';
		END
		ELSE IF(@RETURN_NO='008')
		BEGIN
			-- User has already voted in WINK+ Play
	...
	BEGIN 
		set @successMsg = 'You can now vote! Choose only one video from each category!';
		set @default_image_id = 477;
		SELECT @SMALL_IMAGE_URL = small_image_name ,@SMALL_WEBSITE_URL = small_image_url 
		FROM campaign_small_image 
		WHERE CAMPAIGN_ID = @CAMPAIGN_ID
		and id= @default_image_id
