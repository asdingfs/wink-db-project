CREATE PROCEDURE [dbo].[Update_Customer_By_iMOB_ID]
	(@imob_customer_id varchar(100),
...
DECLARE @OLD_EMAIL VARCHAR(50)
	IF EXISTS (SELECT customer.customer_id FROM CUSTOMER WHERE LTRIM(RTRIM(customer.imob_customer_id)) = @imob_customer_id)
		BEGIN
		SET @OLD_EMAIL = (SELECT CUSTOMER.email FROM customer WHERE customer.imob_customer_id = @imob_customer_id)	
		IF (@password IS NOT NULL AND @password !='')
			...
			BEGIN
                ...
                Where LTRIM(RTRIM(customer.imob_customer_id)) = @imob_customer_id
...
select * from master_user_group_relationship
