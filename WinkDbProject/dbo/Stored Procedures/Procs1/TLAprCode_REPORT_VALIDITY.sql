CREATE PROC [dbo].[TLAprCode_REPORT_VALIDITY]
(
...
BEGIN
	...
	IF(@CAMPAIGN_ID = 160)
	BEGIN
		SELECT TEMP.no,TEMP.gender,TEMP.age,TEMP.wid, TEMP.[status], TEMP.created_at, TEMP.Q1 FROM 
		(--1 START
			...
			(
				-----table1
				SELECT * from
				(SELECT campaign_id,customer_id,created_at, 
				'' as Q1
