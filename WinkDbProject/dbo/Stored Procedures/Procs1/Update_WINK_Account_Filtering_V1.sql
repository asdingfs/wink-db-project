...
CREATE PROCEDURE  [dbo].[Update_WINK_Account_Filtering_V1] 
(
    @customer_id int ,
...
DECLARE 
	...
	-------------1. GET THE ACTION USER ROLE 
		...
		BEGIN
			...
			SET @internal_procedure = ISNULL((SELECT internal_procedure from wink_account_filtering_status_new where 
			filtering_status_key = @filtering_status) ,0) 
			...
			IF (@from_customer_detail ='No')
				BEGIN
					...
					---3.3 -- CHECK MULTIPLE ACCOUNT BY WHATSAPP PHONE NO.--------
					  ...
					  IF(@Final_approval_status ='confiscation_and_unlock_approve' OR 
					  
					  @Final_approval_status ='confiscation_and_1MS_approve'
					  OR @Final_approval_status ='confiscation_and_3M_s_approve'
					  OR @Final_approval_status ='confiscation_and_permanent_s_approve'
					  ...
					  BEGIN							
							...
							ops_manager_action_status ='confiscation_and_permanent_s'))
			  ...
			  UPDATE [dbo].[wink_account_filtering]
			   SET [whatsapp_phone_no] =@old_whatsapp_phone_no
				  ...
				  ,ops_staff_action_status = @ops_staff_action_status
