CREATE PROC [dbo].[WINKTAG_GET_THEMATIC_SIMPLIFIED_REPORT_VALIDITY]
(
...
BEGIN
	...
	IF (@CAMPAIGN_ID = 146)
	BEGIN
	EXEC [dbo].[WINKTAG_GET_THEMATIC_SIMPLIFIED_REPORT_VALIDITY_WFH2020] 
	@customer_name ,
	@email,
	@gender,
	@customer_id,
	...
	@validity 
	END
	ELSE IF(@CAMPAIGN_ID = 138 or @CAMPAIGN_ID = 137 or @CAMPAIGN_ID = 136 or @CAMPAIGN_ID = 116 )
		BEGIN
			SELECT * FROM 
				(--1 START
					...
					(
						-----table1
						SELECT * from
						(SELECT customer_id,location as GPS_location,ip_address,created_at, '' as Q1, '' as Redirection
