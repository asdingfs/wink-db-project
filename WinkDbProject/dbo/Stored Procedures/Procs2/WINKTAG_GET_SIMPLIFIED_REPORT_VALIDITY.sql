CREATE PROC [dbo].[WINKTAG_GET_SIMPLIFIED_REPORT_VALIDITY]
(
...
BEGIN
	...
	/*IF(@CAMPAIGN_ID = 167 )*/
	IF(@CAMPAIGN_ID = 178 or @CAMPAIGN_ID = 179 or @CAMPAIGN_ID = 180 or @CAMPAIGN_ID = 181 or @CAMPAIGN_ID = 182)
	BEGIN
		SELECT * FROM 
			(--1 START
				...
				WHERE @validity is null 
						...
						or (@validity = '0' AND T.Q1_1 = '' AND T.Q1_2 = '' AND T.Q1_3 = '' AND T.Q1_4 = '')
				...
				(--1 START
					...
					WHERE ( @validity is null 
						  ...
						  AND 	
						   ( @winner is null   
							or 
							(@CAMPAIGN_ID = 167  AND ((@winner = 'Yes' AND T.Q1 like '%THE ROAD AHEAD%')  
							or (@winner = '0'  AND T.Q1 not like '%THE ROAD AHEAD%')))
							)
