CREATE PROC [dbo].[WINKTAG_GET_TLMC_SIMPLIFIED_REPORT]
(
...
BEGIN
	...
	BEGIN
		...
		(--1 START
			SELECT CUSTOMER.first_name +' '+CUSTOMER.last_name as customer_name,CUSTOMER.email,CUSTOMER.gender,(select floor(datediff(day,CUSTOMER.date_of_birth, DATEADD(HOUR,8,GETDATE())) / 365.25)) as age,Customer.WID as wid, Customer.[status] as [status],
			...
			(
				-----table1
				SELECT * from
				(SELECT customer_id,[location] as GPS_location,ip_address,created_at, '' as Q1, campaign_id
