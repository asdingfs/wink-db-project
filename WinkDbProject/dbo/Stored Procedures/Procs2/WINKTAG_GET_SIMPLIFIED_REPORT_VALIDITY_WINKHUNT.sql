CREATE PROCEDURE [dbo].[WINKTAG_GET_SIMPLIFIED_REPORT_VALIDITY_WINKHUNT]
...
BEGIN
    ...
    IF(@CAMPAIGN_ID = 211 or @CAMPAIGN_ID = 212 or @CAMPAIGN_ID = 215 or @CAMPAIGN_ID =218  )
        BEGIN
	...
	ELSE IF(@CAMPAIGN_ID=217)
		...
		print('217')
				...
				(--1 START
					SELECT ROW_NUMBER() OVER (Order by T.CREATED_AT ASC)AS no,CUSTOMER.first_name +' '+CUSTOMER.last_name as customer_name,CUSTOMER.email,CUSTOMER.gender,(select floor(datediff(day,CUSTOMER.date_of_birth, DATEADD(HOUR,8,GETDATE())) / 365.25)) as age,Customer.WID as wid, Customer.status as status,
					T.customer_id,T.points,'' as user_type,T.Q1 as promo_code,T.GPS_location,T.ip_address,T.created_at,
					T.Q1
					...
					(
						...
						-----table2
						SELECT C.customer_id,C.points,@user_type as user_type,''as promo_code,C.GPS_location,C.ip_address,C.created_at,
						(
