CREATE PROC [dbo].[WINKTAG_SCRATCH_CARD_SIMPLIFIED_REPORT]
(
		...
		BEGIN
			SELECT * FROM 
				(--1 START
					...
					T.customer_id,T.GPS_location,T.ip_address,T.created_at,T.Q1, T.validity
					
					FROM

					(
						SELECT AL.customer_id, AL.location as GPS_location,AL.ip_address,AL.created_at, '0' as validity,
						'' as Q1
